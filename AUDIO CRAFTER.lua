-- ============================================================
-- AC AudioCrafter  V4.80  by MelodyCrafter
-- !! CLAUDE: EVERY SINGLE EDIT = bump AC_VER + add changelog !!
-- ============================================================
-- CHANGELOG
-- v4.80  (current)
--   FIX: dxni far-distance logo circle enlarged from 32x32 to 52x52
--        to match AK-style tag sizing (visually matches player nametag size)
-- v4.79
--   FIX: dxni logo circle image now fills the full circle (ScaleType.Crop,
--        size 1,0,1,0 + UICorner radius 1,0) instead of sitting as a box
-- v4.78
...
pcall(function()
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "AudioCrafter";
        Text = "v4.80 Attached!  by MelodyCrafter";
...
do
    ...
    local DEFAULTS = {
        toggleKey="G", walkSpeed=16, jumpPower=50, infJump=false, noclip=false,
        fullbright=false, timeOfDay=14, gravity=196, fpsUnlock=false,
        antiAfk=false, antiVoid=false, clickTp=false, tagsVisible=true,
    }
    local function cfgRead()
        local ok,data=pcall(function()
            if not isfile then return nil end; if not isfile(CFG_FILE) then return nil end
            return AC.Http:JSONDecode(readfile(CFG_FILE))
        end); if ok and type(data)=="table" then return data end; return {}
    end
    ...
    local saved=cfgRead(); AC.cfg={}
    for k,v in pairs(DEFAULTS) do AC.cfg[k]=(saved[k]~=nil) and saved[k] or v end
    AC.cfgSave=function() cfgWrite(AC.cfg) end
    AC.cfgToggle=function(key,onCb) return function(v) AC.cfg[key]=v; AC.cfgSave(); onCb(v) end end
    AC.cfgSlider=function(key,onCb) return function(v) AC.cfg[key]=v; AC.cfgSave(); onCb(v) end end
...
do
    ...
    AC.toast=function(msg,color)
        ...
        local f=Instance.new("Frame",AC.toastContainer); f.Size=UDim2.new(0,280,0,0); f.BackgroundColor3=Color3.fromRGB(16,16,16); f.BackgroundTransparency=0.35; f.BorderSizePixel=0; f.ClipsDescendants=true
...
do
    ...
    AC.reopenBtn=Instance.new("TextButton",AC.screenGui); AC.reopenBtn.Size=UDim2.new(0,46,0,46); AC.reopenBtn.Position=UDim2.new(0,100,0,100); AC.reopenBtn.BackgroundColor3=AC.BG_PANEL; AC.reopenBtn.Text=""; AC.reopenBtn.Visible=false; AC.reopenBtn.ZIndex=20
    ...
    local rbDrag=false; local rbDS=nil; local rbSP=nil; local rbMoved=false
    AC.reopenBtn.InputBegan:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.MouseButton1 then
            rbDrag=true; rbMoved=false; rbDS=i.Position
            local vp=AC.camera.ViewportSize; local curX=AC.reopenBtn.Position.X.Offset+AC.reopenBtn.Position.X.Scale*vp.X; local curY=AC.reopenBtn.Position.Y.Offset+AC.reopenBtn.Position.Y.Scale*vp.Y
            rbSP=Vector2.new(curX,curY); i.Changed:Connect(function() if i.UserInputState==Enum.UserInputState.End then rbDrag=false end end)
        end
    end)
    AC.UIS.InputChanged:Connect(function(i)
        if rbDrag and i.UserInputType==Enum.UserInputType.MouseMovement then
            local d=i.Position-rbDS; if d.Magnitude>4 then rbMoved=true end
            if rbMoved then local vp=AC.camera.ViewportSize; AC.reopenBtn.Position=UDim2.new(0,math.clamp(rbSP.X+d.X,0,vp.X-46),0,math.clamp(rbSP.Y+d.Y,0,vp.Y-46)) end
        end
    end)
    AC.reopenBtn.MouseButton1Click:Connect(function() if not rbMoved then AC.doOpen() end; rbMoved=false end)
    ...
    AC.doMin=function()
        ...
        task.delay(0.32,function()
            ...
            local vp=AC.camera.ViewportSize; AC.reopenBtn.Position=UDim2.new(0,vp.X-62,0,vp.Y/2-23); AC.reopenBtn.Visible=true; _uiTweening=false
        end)
    end
    AC.doOpen=function()
        if AC._uiOpen or _uiTweening then return end
        AC._uiOpen=true; _uiTweening=true; AC.reopenBtn.Visible=false; AC.wrapper.Visible=true
        local lastPos=AC.wrapper.Position; local isFirstOpen=(lastPos.X.Scale==0.5 and lastPos.X.Offset==0)
        AC.wrapper.Size=UDim2.new(0,0,0,0); if isFirstOpen then AC.wrapper.Position=UDim2.new(0.5,0,0.5,0) end
        pcall(function() AC.openSound:Play() end)
        local targetPos=isFirstOpen and UDim2.new(0.5,-AC.UI_W/2,0.5,-AC.UI_H/2) or UDim2.new(lastPos.X.Scale,lastPos.X.Offset,lastPos.Y.Scale,lastPos.Y.Offset)
        AC.TS:Create(AC.wrapper,TweenInfo.new(0.42,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Size=UDim2.new(0,AC.UI_W,0,AC.UI_H),Position=targetPos}):Play()
        ...
        while true do task.wait(3)
            pcall(function()
                if not AC.wrapper.Visible and not AC.reopenBtn.Visible and not _uiTweening then
                    local vp=AC.camera.ViewportSize; AC.reopenBtn.Position=UDim2.new(0,vp.X-62,0,vp.Y/2-23); AC.reopenBtn.Visible=true; AC._uiOpen=false
                end
            end)
        end
    end)
    ...
    AC.UIS.InputBegan:Connect(function(i,gp)
        pcall(function()
            if gp then return end; if i.UserInputType~=Enum.UserInputType.Keyboard then return end
            local kn=i.KeyCode.Name; if not kn or kn=="" or kn=="Unknown" then return end
            if kn==(AC._toggleKey or "G") then if AC._uiOpen then AC.doMin() else AC.doOpen() end end
        end)
    end)
end
-- FPS/PING HUD
do
    ...
    tagsBtn.MouseButton1Click:Connect(function()
        AC.tagsVisible=not AC.tagsVisible; AC.cfg.tagsVisible=AC.tagsVisible; AC.cfgSave()
        local clean={}
        ...
        for _,p in ipairs(AC.Players:GetPlayers()) do
            if p.Character then local h=p.Character:FindFirstChild("Head"); if h then for _,bb in ipairs(h:GetChildren()) do
                if bb:IsA("BillboardGui") and (bb.Name=="AC_Billboard" or bb.Name=="AC_Billboard_Logo") then
                    if bb.Name=="AC_Billboard" then bb.Enabled=AC.tagsVisible end
                    if bb.Name=="AC_Billboard_Logo" then bb.Enabled=false end
                end
            end end end
        end
    ...
    local fpsWindow={}; local FPS_WINDOW=20; local fpsUpdateTimer=0; local FPS_UPDATE_RATE=0.1
    local pingUpdateTimer=0; local PING_UPDATE_RATE=0.5; local lastPing=0
    local function getPing()
        local ok1,v1=pcall(function() return math.floor(AC.Stats.Network.ServerStatsItem["Data Ping"]:GetValue()) end)
        if ok1 and v1 and v1>0 and v1<2000 then return v1 end
        local ok3,v3=pcall(function() local t0=os.clock(); local _=AC.WS:GetServerTimeNow(); local t1=os.clock(); return math.floor((t1-t0)*500) end)
    ...
    AC.RS.RenderStepped:Connect(function(dt)
        ...
        fpsWindow[#fpsWindow+1]=dt; if #fpsWindow>FPS_WINDOW then table.remove(fpsWindow,1) end
        fpsUpdateTimer=fpsUpdateTimer+dt
        if fpsUpdateTimer>=FPS_UPDATE_RATE and #fpsWindow>0 then
            fpsUpdateTimer=0; local sorted={}; for _i,_v in ipairs(fpsWindow) do sorted[_i]=_v end; table.sort(sorted)
            local medDt=sorted[math.ceil(#sorted/2)]; local fps=medDt>0 and math.floor(1/medDt+0.5) or 0
        ...
        if pingUpdateTimer>=PING_UPDATE_RATE then
            pingUpdateTimer=0; local ping=getPing()
            if ping then lastPing=ping; pingLbl.Text="PING  "..ping.."ms"; pingLbl.TextColor3=ping<80 and AC.PUR_GLOW or ping<150 and AC.ORANGE_W or AC.RED_ERR
            elseif lastPing==0 then pingLbl.Text="PING  --"; pingLbl.TextColor3=AC.TXT_DIM end
        end
    end)
end
...
do
    ...
    local OWNER_ID=8798325211; local OWNER_NAME="MelodyCrafter3"
    ...
    local function attachTag(char,owner)
        ...
        local isOwner=isOwnerPlayer(owner)
        ...
        local titleText=isOwner and "AC OWNER" or "AC USER"
        ...
        local tagYOffset=2.0
        pcall(function()
            if owner and owner.Character then
                local hrp=owner.Character:FindFirstChild("HumanoidRootPart"); local hum=owner.Character:FindFirstChildOfClass("Humanoid")
                if hrp and hum then tagYOffset=math.clamp(hum.HipHeight*0.5+0.8,0.8,3.0) end
            end
        end)
        local bbFull=Instance.new("BillboardGui")
        bbFull.Name="AC_Billboard"; bbFull.Size=UDim2.new(0,TAG_W,0,TAG_H)
        bbFull.StudsOffset=Vector3.new(0,tagYOffset,0); bbFull.AlwaysOnTop=true
        bbFull.ResetOnSpawn=false; bbFull.MaxDistance=200; bbFull.Adornee=head
        bbFull.Enabled=AC.tagsVisible; bbFull.Parent=head
        table.insert(AC.allBillboards,bbFull)
        local bbLogo=Instance.new("BillboardGui")
        bbLogo.Name="AC_Billboard_Logo"
        -- v4.80: 52x52 to match AK-style tag size
        bbLogo.Size=UDim2.new(0,LOGO_W,0,LOGO_H)
        bbLogo.StudsOffset=Vector3.new(0,tagYOffset,0)
        bbLogo.AlwaysOnTop=true; bbLogo.ResetOnSpawn=false
        bbLogo.MaxDistance=800; bbLogo.Adornee=head; bbLogo.Enabled=false; bbLogo.Parent=head
        local isDxni=(owner and owner.Name=="robloxguycoolestman")
        local DXNI_IMG="rbxthumb://type=Asset&id=108337467293397&w=150&h=150"
        -- Full tag
        local pill=Instance.new("Frame",bbFull); pill.Size=UDim2.new(1,0,1,0); pill.BackgroundColor3=bgColor; pill.BackgroundTransparency=0.15
        ...
        if isDxni then
            local iconImg=Instance.new("ImageLabel",pill); iconImg.Size=UDim2.new(0,40,1,0); iconImg.Position=UDim2.new(0,0,0,0); iconImg.BackgroundTransparency=1; iconImg.ScaleType=Enum.ScaleType.Crop; iconImg.ImageTransparency=0; iconImg.Image=DXNI_IMG; Instance.new("UICorner",iconImg).CornerRadius=UDim.new(0,8)
        else
        ...
        -- Logo circle (far billboard)
        -- v4.79+: fills full circle, v4.80: 52x52 size
        local logoPill=Instance.new("Frame",bbLogo); logoPill.Size=UDim2.new(1,0,1,0); logoPill.BackgroundColor3=bgColor; logoPill.BackgroundTransparency=0.15; logoPill.Visible=false
        ...
        if isDxni then
            -- v4.79 fix: fill circle, crop image, clip to circle shape
            local iconImg2=Instance.new("ImageLabel",logoPill)
            iconImg2.Size=UDim2.new(1,0,1,0)          -- fill entire pill
            iconImg2.Position=UDim2.new(0,0,0,0)
            iconImg2.BackgroundTransparency=1
            iconImg2.Image=DXNI_IMG
            iconImg2.ScaleType=Enum.ScaleType.Crop     -- crop to fill, no letterbox
            iconImg2.ImageTransparency=0
            Instance.new("UICorner",iconImg2).CornerRadius=UDim.new(1,0)
        else
            local logoOnly=AC.drawLogo(logoPill,24,tagColor); logoOnly.Position=UDim2.new(0.5,-12,0.5,-12)
        end
        local function doTp()
            if owner and owner~=AC.player and owner.Character then
                ...
                if r and m then
                    local rightCF=r.CFrame*CFrame.new(3,0,0); local leftCF=r.CFrame*CFrame.new(-3,0,0); local rightBlocked=false
                    for _,p in ipairs(AC.Players:GetPlayers()) do if p~=AC.player and p~=owner and p.Character then local pr=p.Character:FindFirstChild("HumanoidRootPart"); if pr and (pr.Position-rightCF.Position).Magnitude<2.5 then rightBlocked=true; break end end end
                    m.CFrame=rightBlocked and leftCF or rightCF
                    AC.toast("TP'd to "..owner.Name.." ("..(rightBlocked and "left" or "right").." side)",AC.PUR_BRIGHT)
                end
            end
        end
        if not AC._tagRegistry then AC._tagRegistry={} end
        AC._tagRegistry[owner and owner.Name or ""]={head=head,doTp=doTp,owner=owner,yOff=tagYOffset}
        bbFull.Enabled=AC.tagsVisible; bbLogo.Enabled=false; logoPill.Visible=false
        local FULL_DIST=80; local LOGO_DIST=90
        task.spawn(function()
            local state="full"
            while head and head.Parent do
                task.wait(0.3)
                if not AC.tagsVisible then bbFull.Enabled=false; bbLogo.Enabled=false; logoPill.Visible=false
                else
                    local myChar=AC.player.Character; local myRoot=myChar and myChar:FindFirstChild("HumanoidRootPart")
                    if myRoot then
                        local dist=(myRoot.Position-head.Position).Magnitude
                        if state=="full" and dist>LOGO_DIST then state="logo"; bbFull.Enabled=false; bbLogo.Enabled=true; logoPill.Visible=true
                        elseif state=="logo" and dist<FULL_DIST then state="full"; bbLogo.Enabled=false; logoPill.Visible=false; bbFull.Enabled=true end
                    end
                end
            end
            pcall(function() if AC._tagRegistry and owner then AC._tagRegistry[owner.Name or ""]=nil end end)
            pcall(function() bbFull:Destroy() end); pcall(function() bbLogo:Destroy() end)
        end)
    end
    AC._tagRegistry={}
    AC.UIS.InputBegan:Connect(function(inp,gp)
        if gp then return end; if inp.UserInputType~=Enum.UserInputType.MouseButton1 then return end
        local mp=AC.UIS:GetMouseLocation(); local bestDist=math.huge; local bestTp=nil
        for _,entry in pairs(AC._tagRegistry) do
            pcall(function()
                if not entry.head or not entry.head.Parent then return end
                local tagPos=entry.head.Position+Vector3.new(0,entry.yOff or 2.0,0)
                ...
                if screenDist<=200 and screenDist<bestDist then bestDist=screenDist; bestTp=entry.doTp end
            end)
        end
    ...
    local function notifyACUser(p)
        ...
        local bgColor=pIsOwner and Color3.fromRGB(18,10,0) or Color3.fromRGB(8,2,18)
        local titleText=pIsOwner and "AC OWNER" or "AC USER"; local titleColor=pIsOwner and Color3.fromRGB(255,215,60) or AC.PUR_GLOW
        local subText="@"..p.Name.." joined"; local dispName=(p.DisplayName and p.DisplayName~=p.Name) and p.DisplayName or ""
        local NOTIF_W=260; local NOTIF_H=60; local yOff=_notifOffset; _notifOffset=_notifOffset+NOTIF_H+8
        local f=Instance.new("Frame",AC.screenGui); f.Size=UDim2.new(0,NOTIF_W,0,NOTIF_H); f.BackgroundColor3=bgColor; f.BackgroundTransparency=0.35; f.ZIndex=300; f.ClipsDescendants=false
        Instance.new("UICorner",f).CornerRadius=UDim.new(0,10)
        local stroke=Instance.new("UIStroke",f); stroke.Color=accentColor; stroke.Thickness=1.2; stroke.Transparency=0.2
        if pIsOwner then AC.TS:Create(stroke,TweenInfo.new(1.0,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),{Transparency=0.6}):Play() end
        local bar=Instance.new("Frame",f); bar.Size=UDim2.new(0,3,1,-8); bar.Position=UDim2.new(0,0,0,4); bar.BackgroundColor3=accentColor; bar.BorderSizePixel=0; bar.ZIndex=301; Instance.new("UICorner",bar).CornerRadius=UDim.new(1,0)
        local thumb=Instance.new("ImageLabel",f); thumb.Size=UDim2.new(0,40,0,40); thumb.Position=UDim2.new(0,8,0.5,-20); thumb.BackgroundColor3=Color3.fromRGB(15,15,15); thumb.BackgroundTransparency=0.4; thumb.ZIndex=301
        Instance.new("UICorner",thumb).CornerRadius=UDim.new(0,8); Instance.new("UIStroke",thumb).Color=accentColor
        pcall(function() local imgUrl,_=AC.Players:GetUserThumbnailAsync(p.UserId,Enum.ThumbnailType.HeadShot,Enum.ThumbnailSize.Size48x48); thumb.Image=imgUrl end)
        local logoF=AC.drawLogo(f,12,accentColor); logoF.Position=UDim2.new(0,52,0,5); logoF.ZIndex=301
        local titleLbl=Instance.new("TextLabel",f); titleLbl.Size=UDim2.new(1,-118,0,18); titleLbl.Position=UDim2.new(0,66,0,5); titleLbl.BackgroundTransparency=1; titleLbl.Text=titleText; titleLbl.TextColor3=titleColor; titleLbl.TextSize=12; titleLbl.Font=Enum.Font.GothamBold; titleLbl.TextXAlignment=Enum.TextXAlignment.Left; titleLbl.ZIndex=301
        local subLbl=Instance.new("TextLabel",f); subLbl.Size=UDim2.new(1,-118,0,12); subLbl.Position=UDim2.new(0,66,0,24); subLbl.BackgroundTransparency=1; subLbl.Text=subText; subLbl.TextColor3=pIsOwner and Color3.fromRGB(200,160,60) or Color3.fromRGB(170,140,200); subLbl.TextSize=9; subLbl.Font=Enum.Font.Gotham; subLbl.TextXAlignment=Enum.TextXAlignment.Left; subLbl.TextTruncate=Enum.TextTruncate.AtEnd; subLbl.ZIndex=301
        local dnLbl=Instance.new("TextLabel",f); dnLbl.Size=UDim2.new(1,-118,0,10); dnLbl.Position=UDim2.new(0,66,0,38); dnLbl.BackgroundTransparency=1; dnLbl.Text=dispName; dnLbl.TextColor3=pIsOwner and Color3.fromRGB(160,120,40) or Color3.fromRGB(130,100,160); dnLbl.TextSize=8; dnLbl.Font=Enum.Font.Gotham; dnLbl.TextXAlignment=Enum.TextXAlignment.Left; dnLbl.ZIndex=301
        local tpBtn=Instance.new("TextButton",f); tpBtn.Size=UDim2.new(0,44,0,32); tpBtn.Position=UDim2.new(1,-50,0.5,-16); tpBtn.BackgroundColor3=accentColor; tpBtn.BackgroundTransparency=0.3; tpBtn.Text="TP"; tpBtn.TextColor3=Color3.new(1,1,1); tpBtn.TextSize=11; tpBtn.Font=Enum.Font.GothamBold; tpBtn.ZIndex=302; Instance.new("UICorner",tpBtn).CornerRadius=UDim.new(0,8)
        tpBtn.MouseButton1Click:Connect(function()
            if p.Character then local r=p.Character:FindFirstChild("HumanoidRootPart"); local m=AC.player.Character and AC.player.Character:FindFirstChild("HumanoidRootPart")
                if r and m then local rightCF=r.CFrame*CFrame.new(3,0,0); local leftCF=r.CFrame*CFrame.new(-3,0,0); local rightBlocked=false
                    for _,pl in ipairs(AC.Players:GetPlayers()) do if pl~=AC.player and pl~=p and pl.Character then local pr=pl.Character:FindFirstChild("HumanoidRootPart"); if pr and (pr.Position-rightCF.Position).Magnitude<2.5 then rightBlocked=true; break end end end
                    m.CFrame=rightBlocked and leftCF or rightCF; AC.toast("TP'd to "..p.Name,AC.PUR_BRIGHT) end
            else AC.toast(p.Name.." not spawned yet",AC.ORANGE_W) end
        end)
        f.Position=UDim2.new(1,10,0,yOff)
        AC.TS:Create(f,TweenInfo.new(0.3,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{Position=UDim2.new(1,-NOTIF_W-10,0,yOff)}):Play()
    ...
    AC.Players.PlayerRemoving:Connect(function(p)
        ...
        local pIsOwner=isOwnerPlayer(p); local accentColor=pIsOwner and Color3.fromRGB(255,200,40) or AC.PUR_BRIGHT
        local bgColor=pIsOwner and Color3.fromRGB(18,10,0) or Color3.fromRGB(8,2,18)
        local NOTIF_W=260; local NOTIF_H=46; local yOff=_notifOffset; _notifOffset=_notifOffset+NOTIF_H+8
        local f=Instance.new("Frame",AC.screenGui); f.Size=UDim2.new(0,NOTIF_W,0,NOTIF_H); f.BackgroundColor3=bgColor; f.BackgroundTransparency=0.35; f.ZIndex=300; f.ClipsDescendants=false
        Instance.new("UICorner",f).CornerRadius=UDim.new(0,10); local stroke=Instance.new("UIStroke",f); stroke.Color=accentColor; stroke.Thickness=1.2; stroke.Transparency=0.4
        local bar=Instance.new("Frame",f); bar.Size=UDim2.new(0,3,1,-8); bar.Position=UDim2.new(0,0,0,4); bar.BackgroundColor3=accentColor; bar.BorderSizePixel=0; bar.ZIndex=301; Instance.new("UICorner",bar).CornerRadius=UDim.new(1,0)
        local logoF=AC.drawLogo(f,12,accentColor); logoF.Position=UDim2.new(0,8,0.5,-6); logoF.ZIndex=301
        local leaveLbl=Instance.new("TextLabel",f); leaveLbl.Size=UDim2.new(1,-30,1,0); leaveLbl.Position=UDim2.new(0,24,0,0); leaveLbl.BackgroundTransparency=1; leaveLbl.Text=(pIsOwner and "AC OWNER" or "AC USER").." LEFT  @"..p.Name; leaveLbl.TextColor3=pIsOwner and Color3.fromRGB(200,150,40) or Color3.fromRGB(150,120,190); leaveLbl.TextSize=11; leaveLbl.Font=Enum.Font.GothamBold; leaveLbl.TextXAlignment=Enum.TextXAlignment.Left; leaveLbl.ZIndex=301
        f.Position=UDim2.new(1,10,0,yOff)
        AC.TS:Create(f,TweenInfo.new(0.3,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{Position=UDim2.new(1,-NOTIF_W-10,0,yOff)}):Play()
        task.delay(6,function() if f and f.Parent then AC.TS:Create(f,TweenInfo.new(0.25,Enum.EasingStyle.Quint,Enum.EasingDirection.In),{Position=UDim2.new(1,10,0,yOff)}):Play(); task.delay(0.3,function() if f and f.Parent then f:Destroy() end; _notifOffset=math.max(10,_notifOffset-(NOTIF_H+8)) end) end end)
    end)
...
do
    ...
    local ws=Instance.new("TextLabel",wCard); ws.Size=UDim2.new(1,-20,0,20); ws.Position=UDim2.new(0,14,0,44); ws.BackgroundTransparency=1; ws.Text="AC AudioCrafter v4.80  by MelodyCrafter"; ws.TextColor3=AC.PUR_MID; ws.TextSize=12; ws.Font=Enum.Font.Gotham; ws.TextXAlignment=Enum.TextXAlignment.Left
    ...
    ic("Version","v4.80",0); ic("Status","* Active",1,AC.GREEN_OK); ic("Script",AC.executorName,2,AC.PUR_BRIGHT)
    ...
    cll("v4.80  Current",12,AC.PUR_GLOW,Enum.Font.GothamBold)
    cll("FIX: Far-distance logo circle enlarged 32->52px to match AK nametag size.",10,AC.TXT_MAIN)
    cll("v4.79",12,AC.PUR_MID,Enum.Font.GothamBold)
    cll("FIX: dxni logo circle image fills the full circle (ScaleType.Crop + UICorner).",10,AC.TXT_MAIN)
    cll("v4.78",12,AC.PUR_MID,Enum.Font.GothamBold)
    ...
    cll("MOVED: Infinite Baseplate moved to World tab.",10,AC.TXT_MAIN)
    ...
    cll("FIX: Open anim no longer shrinks sidebar/mainPanel.",10,AC.TXT_MAIN)
    cll("FIX: Only wrapper animates - inner panels stay full size.",10,AC.TXT_MAIN)
...
do
        ...
        for _,p in ipairs(AC.Players:GetPlayers()) do
            if p~=AC.player then
                if q=="" or p.Name:lower():find(q,1,true) or p.DisplayName:lower():find(q,1,true) then
                    ...
                    local pCopy=p
                    row.MouseButton1Click:Connect(function() ddSelecting=true; AC.selectedTarget=pCopy; sBox.Text=pCopy.Name; ddFrame.Visible=false; sBox:ReleaseFocus(); AC.toast("Target: "..pCopy.Name,AC.PUR_BRIGHT); task.delay(0.5,function() ddSelecting=false end) end)
    ...
    local sBoxFocused=false
    sBox.Focused:Connect(function() sBoxFocused=true; if not ddSelecting then rebuildDropdown(sBox.Text) end end)
    sBox.FocusLost:Connect(function() sBoxFocused=false; task.wait(0.4); if not ddSelecting then ddFrame.Visible=false end end)
    sBox:GetPropertyChangedSignal("Text"):Connect(function()
        if ddSelecting then return end
        if sBoxFocused then if sBox.Text=="" then AC.selectedTarget=nil end; rebuildDropdown(sBox.Text) else ddFrame.Visible=false end
    end)
    AC.Players.PlayerRemoving:Connect(function(p) if AC.selectedTarget==p then AC.selectedTarget=nil; sBox.Text="" end end)
    AC.UIS.InputBegan:Connect(function(i) pcall(function() if i.UserInputType==Enum.UserInputType.MouseButton1 and ddFrame.Visible and not ddSelecting then task.wait(0.15); if not ddSelecting then ddFrame.Visible=false end end end) end)
    ...
    if AC.cfg.toggleKey and AC.cfg.toggleKey~="G" then AC._toggleKey=AC.cfg.toggleKey; kbValLbl.Text="["..AC.cfg.toggleKey.."]" end
    ...
    kbBtn.MouseButton1Click:Connect(function()
        ...
        _kbConn=AC.UIS.InputBegan:Connect(function(inp,gp)
            ...
            AC._toggleKey=kn; kbValLbl.Text="["..kn.."]"; kbValLbl.TextColor3=AC.GREEN_OK; kbTopLbl.Text="Toggle UI Key"; kbStr.Color=AC.PUR_STROKE
            _kbListening=false; if _kbConn then _kbConn:Disconnect(); _kbConn=nil end
            AC.cfg.toggleKey=kn; AC.cfgSave(); AC.toast("Toggle key set to ["..kn.."]",AC.PUR_BRIGHT); task.delay(1.2,function() kbValLbl.TextColor3=AC.PUR_GLOW end)
        end)
    end)
    ...
    baBtn.MouseButton1Click:Connect(function()
        if AC.inBp then AC.inBp=false; if AC.bpConn then AC.bpConn:Disconnect(); AC.bpConn=nil end; local mh=AC.player.Character and AC.player.Character:FindFirstChildOfClass("Humanoid"); if mh then mh.PlatformStand=false end; baBtn.Text="Back-to-Back"; AC.toast("Back-to-Back OFF",AC.ORANGE_W); return end
        if not AC.selectedTarget or not AC.selectedTarget.Character then AC.toast("No target",AC.RED_ERR); return end
        AC.inBp=true; baBtn.Text="Exit B2B"
        AC.bpConn=AC.RS.Heartbeat:Connect(function()
            if not AC.inBp then return end
            local mr=AC.player.Character and AC.player.Character:FindFirstChild("HumanoidRootPart"); local mh2=AC.player.Character and AC.player.Character:FindFirstChildOfClass("Humanoid")
            local tr=AC.selectedTarget.Character and AC.selectedTarget.Character:FindFirstChild("HumanoidRootPart")
            if mr and tr then if mh2 then mh2.PlatformStand=true end; mr.CFrame=(tr.CFrame*CFrame.new(0,0.5,1.5))*CFrame.Angles(0,math.pi,0) end
        end)
    end)
...
do
    ...
    local _,_,onSh=AC.makeToggle(pg,"Shaders (PShade Ultimate)",218,false)
    onSh(function(v) if v then task.spawn(function() pcall(function() safeRun("PSHADE") end) end); AC.toast("Shaders ON") else AC.toast("Rejoin to disable",AC.ORANGE_W) end end)
    local _,_,onBP=AC.makeToggle(pg,"Infinite Baseplate",264,false)
    onBP(function(v)
        if v then
            local function findFloor()
                local names={"Baseplate","Base","Floor","Ground","Map","Terrain","baseplate","base","floor","ground","map"}
                for _,n in ipairs(names) do local p=AC.WS:FindFirstChild(n,true); if p and p:IsA("BasePart") and p.Size.X>50 then return p,p.Position.Y+(p.Size.Y/2) end end
                local bestPart=nil; local bestArea=0
                for _,obj in ipairs(AC.WS:GetDescendants()) do if obj:IsA("BasePart") and obj.Anchored and obj.Size.Y<=8 then local area=obj.Size.X*obj.Size.Z; if area>bestArea then bestArea=area; bestPart=obj end end end
                if bestPart then return bestPart,bestPart.Position.Y+(bestPart.Size.Y/2) end
                local params=RaycastParams.new(); params.FilterType=Enum.RaycastFilterType.Exclude; if AC.player.Character then params.FilterDescendantsInstances={AC.player.Character} end
                local hits={}; for _,pt in ipairs({{0,0},{200,0},{-200,0},{0,200},{0,-200}}) do local res=AC.WS:Raycast(Vector3.new(pt[1],500,pt[2]),Vector3.new(0,-700,0),params); if res then hits[#hits+1]=res.Position.Y end end
                if #hits>0 then table.sort(hits); return nil,hits[math.ceil(#hits/2)] end
                local root=AC.player.Character and AC.player.Character:FindFirstChild("HumanoidRootPart"); return nil,root and root.Position.Y-3 or 0
            end
            for _,p in ipairs(AC.WS:GetChildren()) do if p.Name=="AC_InfiniteBaseplate" then pcall(function() p:Destroy() end) end end
            local _,floorY=findFloor(); local TILE=2048; local GRID=21; local HALF=math.floor(GRID/2); local tileY=floorY-0.5; local totalTiles=GRID*GRID; local built=0
            local folder=Instance.new("Folder",AC.WS); folder.Name="AC_InfiniteBaseplate"
            local coords={}; for gx=-HALF,HALF do for gz=-HALF,HALF do coords[#coords+1]={gx,gz,gx*gx+gz*gz} end end; table.sort(coords,function(a,b) return a[3]<b[3] end)
            local function makeTile(gx,gz) local bp=Instance.new("Part"); bp.Name="tile"; bp.Size=Vector3.new(TILE,1,TILE); bp.CFrame=CFrame.new(gx*TILE,tileY,gz*TILE); bp.Anchored=true; bp.CanCollide=true; bp.CastShadow=false; bp.Locked=true; bp.Material=Enum.Material.SmoothPlastic; bp.Transparency=1; bp.TopSurface=Enum.SurfaceType.Smooth; bp.BottomSurface=Enum.SurfaceType.Smooth; pcall(function() bp.RenderFidelity=Enum.RenderFidelity.Automatic end); bp.Parent=folder end
            task.spawn(function()
                for i=1,math.min(9,#coords) do makeTile(coords[i][1],coords[i][2]); built=built+1 end
                for i=10,#coords do makeTile(coords[i][1],coords[i][2]); built=built+1; if built%25==0 then task.wait() end end
                AC.toast("Baseplate ON ("..totalTiles.." tiles)",AC.PUR_BRIGHT)
            end)
            AC.baseplateRef=folder; AC.toast("Building baseplate...",AC.ORANGE_W)
        else
            if AC.baseplateRef then pcall(function() AC.baseplateRef:Destroy() end); AC.baseplateRef=nil end
            for _,p in ipairs(AC.WS:GetChildren()) do if p.Name=="AC_InfiniteBaseplate" then pcall(function() p:Destroy() end) end end
    ...
    onFU(AC.cfgToggle("fpsUnlock",function(v)
        if v then
            ...
            pcall(function() for _,e in ipairs(AC.Lighting:GetChildren()) do if e:IsA("PostEffect") then pcall(function() e.Enabled=false end) end end end)
            pcall(function() for _,e in ipairs(AC.Lighting:GetChildren()) do if e:IsA("Sky") then pcall(function() e.Enabled=false end) end end end)
            AC.toast("FPS Boost ON",AC.GREEN_OK)
        else
            pcall(function() setfpscap(60) end); pcall(function() AC.Lighting.GlobalShadows=true end)
            pcall(function() for _,e in ipairs(AC.Lighting:GetChildren()) do if e:IsA("PostEffect") or e:IsA("Sky") then pcall(function() e.Enabled=true end) end end end)
...
do
    ...
    local function ensureAPI(cb)
        ...
        task.spawn(function()
            local ok,result=pcall(function() local src=game:HttpGet(_URLS.REANIMATION); assert(type(src)=="string" and #src>0,"empty"); local fn,err=loadstring(src); assert(fn,"ls: "..tostring(err)); return fn() end)
            rnLoading=false
                ...
                if pOk and parsed and type(parsed)=="table" then
                    ...
                    for _,entry in ipairs(parsed) do
                        if type(entry)=="table" and type(entry.name)=="string" and entry.name:sub(-4)==".lua" and entry.type=="file" then
                            local name=entry.name:sub(1,-5); local encoded=entry.name:gsub("%%","%%25"):gsub(" ","%%20"):gsub("'","%%27"):gsub("%(","%%28"):gsub("%)","%%29"):gsub("!","%%21"):gsub("#","%%23")
    ...
    local rnSpdLbl=Instance.new("TextLabel",rnPanel); rnSpdLbl.Size=UDim2.new(0,55,0,14); rnSpdLbl.Position=UDim2.new(0,8,0,336); rnSpdLbl.BackgroundTransparency=1; rnSpdLbl.Text="Speed:"; rnSpdLbl.TextColor3=AC.TXT_DIM; rnSpdLbl.TextSize=10; rnSpdLbl.Font=Enum.Font.GothamBold; rnSpdLbl.ZIndex=71
    local rnSpdVal=Instance.new("TextLabel",rnPanel); rnSpdVal.Size=UDim2.new(0,30,0,14); rnSpdVal.Position=UDim2.new(1,-68,0,336); rnSpdVal.BackgroundTransparency=1; rnSpdVal.Text="5"; rnSpdVal.TextColor3=AC.TXT_WHITE; rnSpdVal.TextSize=10; rnSpdVal.Font=Enum.Font.GothamBold; rnSpdVal.ZIndex=71
    local rnRstSpd=Instance.new("TextButton",rnPanel); rnRstSpd.Size=UDim2.new(0,40,0,14); rnRstSpd.Position=UDim2.new(1,-8,0,336); rnRstSpd.BackgroundTransparency=1; rnRstSpd.Text="Reset"; rnRstSpd.TextColor3=AC.TXT_DIM; rnRstSpd.TextSize=9; rnRstSpd.Font=Enum.Font.Gotham; rnRstSpd.ZIndex=71
    local rnSpdTrack=Instance.new("Frame",rnPanel); rnSpdTrack.Size=UDim2.new(1,-16,0,6); rnSpdTrack.Position=UDim2.new(0,8,0,354); rnSpdTrack.BackgroundColor3=Color3.fromRGB(30,30,35); rnSpdTrack.ZIndex=71; Instance.new("UICorner",rnSpdTrack).CornerRadius=UDim.new(1,0)
    ...
    local rnAllRows={}; local rnFavOnly=false; local rnFavs={}; local rnBinds={}; local rnSpeeds={}; local rnNowUrl=nil; local rnHighRow=nil
    ...
    local function setRnTogUI(active)
        ...
        if active then
            AC.TS:Create(rnEnTog,TweenInfo.new(0.2),{BackgroundColor3=AC.PUR_MID}):Play(); AC.TS:Create(rnEnKnob,TweenInfo.new(0.2),{Position=UDim2.new(1,-18,0.5,-8),BackgroundColor3=AC.PUR_GLOW}):Play()
            local rnRotConn=AC.RS.RenderStepped:Connect(function() if not rnActive then return end; pcall(function() local char=AC.player.Character; if not char then return end; local mr=char:FindFirstChild("HumanoidRootPart"); if not mr then return end; local _,camY,_=AC.camera.CFrame:ToEulerAnglesYXZ(); mr.CFrame=CFrame.new(mr.CFrame.Position)*CFrame.Angles(0,camY,0) end) end)
            AC._rnRotConn=rnRotConn
        else
            AC.TS:Create(rnEnTog,TweenInfo.new(0.2),{BackgroundColor3=Color3.fromRGB(40,40,40)}):Play(); AC.TS:Create(rnEnKnob,TweenInfo.new(0.2),{Position=UDim2.new(0,2,0.5,-8),BackgroundColor3=AC.TXT_DIM}):Play()
        end
    end
    rnEnTog.MouseButton1Click:Connect(function()
        ensureAPI(function(api)
            if not rnActive then
                local char=AC.player.Character; local hum=char and char:FindFirstChildOfClass("Humanoid"); local oldMH=hum and hum.MaxHealth or 100
                if hum then hum.MaxHealth=math.huge; hum.Health=math.huge end
                local ok,_=pcall(function() api.reanimate(true) end)
                task.delay(0.5,function() pcall(function() local c=AC.player.Character; local h=c and c:FindFirstChildOfClass("Humanoid"); if h then h.MaxHealth=oldMH; h.Health=oldMH end end) end)
                if ok then setRnTogUI(true); rnStatusLbl.Text="Ready"; AC.toast("Reanimation ON",AC.GREEN_OK)
                else pcall(function() if hum then hum.MaxHealth=oldMH; hum.Health=oldMH end end); AC.toast("Reanimate failed",AC.RED_ERR) end
            else
                if rnNowUrl then pcall(function() api.stop_animation() end) end
                if AC._rnRotConn then AC._rnRotConn:Disconnect(); AC._rnRotConn=nil end
                local char=AC.player.Character; local hum=char and char:FindFirstChildOfClass("Humanoid"); local oldMH=hum and hum.MaxHealth or 100
                if hum then hum.MaxHealth=math.huge; hum.Health=math.huge end
                pcall(function() api.reanimate(false) end)
                task.delay(0.3,function() pcall(function() local c=AC.player.Character; local h=c and c:FindFirstChildOfClass("Humanoid"); if h then h.MaxHealth=oldMH; h.Health=oldMH; pcall(function() h:ChangeState(Enum.HumanoidStateType.GettingUp) end); h.PlatformStand=false end end) end)
                setRnTogUI(false); rnNowUrl=nil; if rnHighRow then pcall(function() rnHighRow.BackgroundColor3=Color3.fromRGB(14,14,18) end); rnHighRow=nil end; rnStatusLbl.Text="Ready"; AC.toast("Reanimation OFF",AC.ORANGE_W)
            end
        end)
    end)
    local function buildRnRow(name,url,idx)
        ...
        local rS=Instance.new("UIStroke",row); rS.Color=AC.PUR_STROKE; rS.Thickness=1; rS.Transparency=0.7; local idxStr=tostring(idx)
        ...
        local nLbl=Instance.new("TextLabel",row); nLbl.Size=UDim2.new(1,-80,1,0); nLbl.Position=UDim2.new(0,32,0,0); nLbl.BackgroundTransparency=1; nLbl.Text=name; nLbl.TextColor3=AC.TXT_MAIN; nLbl.TextSize=12; nLbl.Font=Enum.Font.Gotham; nLbl.TextXAlignment=Enum.TextXAlignment.Left; nLbl.TextTruncate=Enum.TextTruncate.AtEnd; nLbl.ZIndex=73
        local bLbl=Instance.new("TextButton",row); bLbl.Size=UDim2.new(0,28,0,14); bLbl.Position=UDim2.new(1,-32,0.5,-7); bLbl.BackgroundColor3=Color3.fromRGB(26,26,32); bLbl.Text=rnBinds[idxStr] and rnBinds[idxStr]:sub(1,3) or "Bind"; bLbl.TextColor3=rnBinds[idxStr] and AC.TXT_WHITE or AC.TXT_DIM; bLbl.TextSize=8; bLbl.Font=Enum.Font.Gotham; bLbl.ZIndex=73; bLbl.BackgroundTransparency=0.3; Instance.new("UICorner",bLbl).CornerRadius=UDim.new(0,4)
        ...
        bLbl.MouseButton1Click:Connect(function()
            ...
            bConn=AC.UIS.InputBegan:Connect(function(inp,gp)
                ...
                if kn=="Escape" or kn=="Delete" or kn=="Backspace" then bListen=false; if bConn then bConn:Disconnect(); bConn=nil end; if kn~="Escape" then rnBinds[idxStr]=nil; saveRnBinds(); bLbl.Text="Bind"; bLbl.TextColor3=AC.TXT_DIM else bLbl.Text=rnBinds[idxStr] and rnBinds[idxStr]:sub(1,3) or "Bind"; bLbl.TextColor3=rnBinds[idxStr] and AC.TXT_WHITE or AC.TXT_DIM end; return end
        ...
        row.MouseButton1Click:Connect(function()
            ...
            ensureAPI(function(api)
                if highlighted then pcall(function() api.stop_animation() end); rnNowUrl=nil; highlighted=false; AC.TS:Create(row,TweenInfo.new(0.15),{BackgroundColor3=Color3.fromRGB(14,14,18)}):Play(); rS.Color=AC.PUR_STROKE; rnHighRow=nil; AC.toast("Stopped: "..nameC,AC.ORANGE_W); return end
                if rnHighRow then pcall(function() AC.TS:Create(rnHighRow,TweenInfo.new(0.15),{BackgroundColor3=Color3.fromRGB(14,14,18)}):Play() end) end
                pcall(function() api.play_animation(urlC,rnSpeeds[idxStr] or 1.0) end); rnNowUrl=urlC; highlighted=true; rnHighRow=row
    ...
    task.spawn(function()
        fetchRnAnimList(function(list)
            ...
            rnScroll.CanvasSize=UDim2.new(0,0,0,#list*36+8); rnStatusLbl.Text="Loaded "..#list.." anims"; AC.toast("Loaded "..#list.." animations",AC.GREEN_OK)
        end)
    end)
    ...
    AC.UIS.InputBegan:Connect(function(inp,gp) pcall(function() if gp or inp.UserInputType~=Enum.UserInputType.Keyboard then return end; local kn=inp.KeyCode.Name; if not kn or kn=="" or kn=="Unknown" then return end; if rnActive then for idxStr,bk in pairs(rnBinds) do if bk==kn then for _,r in ipairs(rnAllRows) do if r and tostring(r.index)==idxStr then ensureAPI(function(api) pcall(function() api.play_animation(r.url,rnSpeeds[idxStr] or 1.0) end); AC.toast("* "..r.nameLower,AC.PUR_BRIGHT) end); end; break end end end end end) end)
    ...
    -- UGC PANEL (simple version)
    local UGC_W,UGC_H=320,420
    ...
    local ugcSrch=Instance.new("TextBox",ugcPanel); ugcSrch.Size=UDim2.new(1,-16,0,26); ugcSrch.Position=UDim2.new(0,8,0,44); ugcSrch.BackgroundColor3=Color3.fromRGB(20,20,24); ugcSrch.BackgroundTransparency=0.3; ugcSrch.PlaceholderText="Search emotes..."; ugcSrch.PlaceholderColor3=AC.TXT_DIM; ugcSrch.Text=""; ugcSrch.TextColor3=AC.TXT_MAIN; ugcSrch.TextSize=12; ugcSrch.Font=Enum.Font.Gotham; ugcSrch.ClearTextOnFocus=false; ugcSrch.ZIndex=66; Instance.new("UICorner",ugcSrch).CornerRadius=UDim.new(0,7); Instance.new("UIStroke",ugcSrch).Color=AC.PUR_STROKE; Instance.new("UIPadding",ugcSrch).PaddingLeft=UDim.new(0,8)
    local ugcScroll=Instance.new("ScrollingFrame",ugcPanel); ugcScroll.Size=UDim2.new(1,-8,1,-140); ugcScroll.Position=UDim2.new(0,4,0,74); ugcScroll.BackgroundTransparency=1; ugcScroll.BorderSizePixel=0; ugcScroll.ScrollBarThickness=3; ugcScroll.ScrollBarImageColor3=AC.PUR_MID; ugcScroll.AutomaticCanvasSize=Enum.AutomaticSize.Y; ugcScroll.CanvasSize=UDim2.new(0,0,0,0); ugcScroll.ZIndex=67; Instance.new("UIListLayout",ugcScroll).Padding=UDim.new(0,2)
    local ugcSpdBar=Instance.new("Frame",ugcPanel); ugcSpdBar.Size=UDim2.new(1,-16,0,52); ugcSpdBar.Position=UDim2.new(0,8,1,-60); ugcSpdBar.BackgroundColor3=Color3.fromRGB(14,14,18); ugcSpdBar.BackgroundTransparency=0.3; ugcSpdBar.ZIndex=66; Instance.new("UICorner",ugcSpdBar).CornerRadius=UDim.new(0,7)
    local ugcSpLbl=Instance.new("TextLabel",ugcSpdBar); ugcSpLbl.Size=UDim2.new(0,55,0,14); ugcSpLbl.Position=UDim2.new(0,8,0,4); ugcSpLbl.BackgroundTransparency=1; ugcSpLbl.Text="Speed:"; ugcSpLbl.TextColor3=AC.TXT_DIM; ugcSpLbl.TextSize=10; ugcSpLbl.Font=Enum.Font.GothamBold; ugcSpLbl.TextXAlignment=Enum.TextXAlignment.Left; ugcSpLbl.ZIndex=67
    local ugcSpVal=Instance.new("TextLabel",ugcSpdBar); ugcSpVal.Size=UDim2.new(0,30,0,14); ugcSpVal.Position=UDim2.new(1,-72,0,4); ugcSpVal.BackgroundTransparency=1; ugcSpVal.Text="5"; ugcSpVal.TextColor3=AC.TXT_WHITE; ugcSpVal.TextSize=10; ugcSpVal.Font=Enum.Font.GothamBold; ugcSpVal.ZIndex=67
    local ugcSpRst=Instance.new("TextButton",ugcSpdBar); ugcSpRst.Size=UDim2.new(0,40,0,14); ugcSpRst.Position=UDim2.new(1,-40,0,4); ugcSpRst.BackgroundTransparency=1; ugcSpRst.Text="Reset"; ugcSpRst.TextColor3=AC.TXT_DIM; ugcSpRst.TextSize=9; ugcSpRst.Font=Enum.Font.Gotham; ugcSpRst.ZIndex=67
    local ugcSpTrk=Instance.new("Frame",ugcSpdBar); ugcSpTrk.Size=UDim2.new(1,-16,0,6); ugcSpTrk.Position=UDim2.new(0,8,0,22); ugcSpTrk.BackgroundColor3=Color3.fromRGB(30,30,35); ugcSpTrk.ZIndex=67; Instance.new("UICorner",ugcSpTrk).CornerRadius=UDim.new(1,0)
    ...
    local ugcAllRows={}; local ugcHighRow=nil
    local function stopUgcTrack()
        if ugcActiveTrack then pcall(function() ugcActiveTrack:Stop(0) end); ugcActiveTrack=nil end
        for _,t in ipairs(ugcLoadedTracks) do pcall(function() if t then t:Stop(0) end end) end; ugcLoadedTracks={}
        pcall(function() local char=AC.player.Character; if not char then return end; local hum=char:FindFirstChildOfClass("Humanoid"); if not hum then return end; local animator=hum:FindFirstChildOfClass("Animator"); if not animator then return end; for _,t in ipairs(animator:GetPlayingAnimationTracks()) do pcall(function() t:Stop(0) end) end end)
    end
    local function playUgcEmote(id)
        stopUgcTrack(); local char=AC.player.Character; if not char then return end; local hum=char:FindFirstChildOfClass("Humanoid"); if not hum then return end
        ...
        local ok,track=pcall(function() return animator:LoadAnimation(anim) end); if not ok or not track then AC.toast("Failed to load emote",AC.RED_ERR); return end
        track.Priority=Enum.AnimationPriority.Action4; track.Looped=true; track:Play(0.3); pcall(function() track:AdjustSpeed(ugcSpdCur) end); ugcActiveTrack=track
    ...
    local function buildUgcRow(item,idx)
        ...
        local row=Instance.new("TextButton",ugcScroll); row.Size=UDim2.new(1,0,0,34); row.BackgroundColor3=Color3.fromRGB(14,14,18); row.BackgroundTransparency=0.1; row.Text=""; row.LayoutOrder=idx; row.ZIndex=68; Instance.new("UICorner",row).CornerRadius=UDim.new(0,6)
        local nLbl=Instance.new("TextLabel",row); nLbl.Size=UDim2.new(1,-44,1,0); nLbl.Position=UDim2.new(0,10,0,0); nLbl.BackgroundTransparency=1; nLbl.Text=item.name; nLbl.TextColor3=AC.TXT_MAIN; nLbl.TextSize=12; nLbl.Font=Enum.Font.Gotham; nLbl.TextXAlignment=Enum.TextXAlignment.Left; nLbl.TextTruncate=Enum.TextTruncate.AtEnd; nLbl.ZIndex=69
        ...
        bLbl.MouseButton1Click:Connect(function()
            ...
            bC=AC.UIS.InputBegan:Connect(function(inp,gp) pcall(function() if gp or inp.UserInputType~=Enum.UserInputType.Keyboard then return end; local kn=inp.KeyCode.Name; if not kn or kn=="" or kn=="Unknown" then return end; ugcBinds[idxStr]=kn; saveUgcBinds(); bLbl.Text=kn:sub(1,3); bLbl.TextColor3=AC.TXT_WHITE; bL=false; if bC then bC:Disconnect(); bC=nil end end) end)
        end)
        local highlighted=false
        row.MouseButton1Click:Connect(function()
            if highlighted then stopUgcTrack(); highlighted=false; ugcHighRow=nil; row.BackgroundColor3=Color3.fromRGB(14,14,18); AC.toast("Stopped",AC.ORANGE_W); return end
            if ugcHighRow then for _,r in ipairs(ugcAllRows) do if r and r.id==ugcHighRow then if r.row and r.row.Parent then r.row.BackgroundColor3=Color3.fromRGB(14,14,18) end; break end end end
            stopUgcTrack(); highlighted=true; ugcHighRow=item.id; row.BackgroundColor3=AC.PUR_DARK; playUgcEmote(item.id); AC.toast("* "..item.name,AC.PUR_BRIGHT)
        end)
    ...
    local function loadUgcData()
        ...
        task.spawn(function()
            local ok,res=pcall(function() return game:HttpGet(_URLS.EMOTE_JSON) end); local emotes={}
            if ok and res and res~="" then local parsed=AC.Http:JSONDecode(res); for _,item in ipairs(parsed.data or {}) do if tonumber(item.id) and tonumber(item.id)>0 then emotes[#emotes+1]={id=tonumber(item.id),name=item.name or ("Emote_"..item.id)} end end end
            ugcStatusLbl.Text="Building ("..#emotes..")"
            for i=1,#emotes do ugcAllRows[i]=buildUgcRow(emotes[i],i); if i%25==0 then ugcScroll.CanvasSize=UDim2.new(0,0,0,i*36+4); task.wait() end end
            ugcScroll.CanvasSize=UDim2.new(0,0,0,#emotes*36+4); ugcStatusLbl.Text="Ready ("..#emotes..")"
        end)
    end
    ugcSrch:GetPropertyChangedSignal("Text"):Connect(function()
        local q=ugcSrch.Text:lower()
        for _,r in ipairs(ugcAllRows) do if r and r.row and r.row.Parent then r.row.Visible=(q=="" or r.nameLower:find(q,1,true)) end end
    end)
    AC.UIS.InputBegan:Connect(function(inp,gp) pcall(function() if gp or inp.UserInputType~=Enum.UserInputType.Keyboard then return end; local kn=inp.KeyCode.Name; if not kn or kn=="" or kn=="Unknown" then return end; for idStr,bk in pairs(ugcBinds) do if bk==kn then local idNum=tonumber(idStr); if idNum then playUgcEmote(idNum) end end end end) end)
    ugcClsBtn.MouseButton1Click:Connect(function()
        ugcPanel.Visible=false; stopUgcTrack()
        for _,r in ipairs(ugcAllRows) do if r and r.row then pcall(function() r.row:Destroy() end) end end
        ugcAllRows={}; ugcScroll.CanvasSize=UDim2.new(0,0,0,0); ugcLoaded=false; ugcHighRow=nil
    end)
    ...
    rnMainBtn.MouseButton1Click:Connect(function() rnPanel.Visible=not rnPanel.Visible; if rnPanel.Visible then local vp=AC.camera.ViewportSize; local pos=rnPanel.Position; if pos.X.Offset<0 or pos.X.Offset>vp.X-RN_W or pos.Y.Offset<0 or pos.Y.Offset>vp.Y-RN_H then rnPanel.Position=UDim2.new(0,math.floor(vp.X/2)-RN_W/2,0,math.floor(vp.Y/2)-RN_H/2) end end end)
    AC.sectionLbl(pg,"UGC EMOTES",78)
    local ugcMainBtn=Instance.new("TextButton",pg); ugcMainBtn.Size=UDim2.new(1,-24,0,40); ugcMainBtn.Position=UDim2.new(0,12,0,96); ugcMainBtn.BackgroundColor3=AC.BG_CARD; ugcMainBtn.Text="Open Emote Menu"; ugcMainBtn.TextColor3=AC.PUR_GLOW; ugcMainBtn.TextSize=14; ugcMainBtn.Font=Enum.Font.GothamBold; Instance.new("UICorner",ugcMainBtn).CornerRadius=UDim.new(0,8); Instance.new("UIStroke",ugcMainBtn).Color=AC.PUR_STROKE
...
do
    ...
    vcMinBtn.MouseButton1Click:Connect(function() vcMinimized=not vcMinimized; if vcMinimized then AC.TS:Create(vcPanel,TweenInfo.new(0.2,Enum.EasingStyle.Quint),{Size=UDim2.new(0,VC_W,0,32)}):Play() else AC.TS:Create(vcPanel,TweenInfo.new(0.2,Enum.EasingStyle.Quint),{Size=UDim2.new(0,VC_W,0,VC_H)}):Play() end end)
    ...
    vcActBtn.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            local ok,err=pcall(function() local vin=game:GetService("VoiceChatInternal"); firesignal(vin.TempSetMicMutedToggleMic); vin:PublishPause(false) end)
    ...
    vcMainBtn.MouseButton1Click:Connect(function()
        pcall(function() AC.clickSnd:Play() end); vcPanel.Visible=not vcPanel.Visible
        if vcPanel.Visible then local vp=AC.camera.ViewportSize; local pos=vcPanel.Position; vcPanel.Position=UDim2.new(0,math.clamp(pos.X.Offset,0,vp.X-VC_W),0,math.clamp(pos.Y.Offset,0,vp.Y-VC_H)) end
    end)
    ...
    local cr1=Instance.new("TextLabel",crd); cr1.Size=UDim2.new(1,-16,0,20); cr1.Position=UDim2.new(0,12,0,6); cr1.BackgroundTransparency=1; cr1.Text="AC AudioCrafter V4.80  by MelodyCrafter"; cr1.TextColor3=AC.PUR_BRIGHT; cr1.TextSize=13; cr1.Font=Enum.Font.GothamBold; cr1.TextXAlignment=Enum.TextXAlignment.Left
...
do
    ...
    local fbPanel=Instance.new("Frame",AC.screenGui); fbPanel.Size=UDim2.new(0,FB_W,0,FB_H); fbPanel.Position=UDim2.new(0,math.floor(AC.camera.ViewportSize.X/2)-FB_W/2,0,math.floor(AC.camera.ViewportSize.Y/2)-FB_H/2); fbPanel.BackgroundColor3=AC.BG_PANEL; fbPanel.BackgroundTransparency=0; fbPanel.ZIndex=80; fbPanel.Visible=false; fbPanel.ClipsDescendants=true; Instance.new("UICorner",fbPanel).CornerRadius=UDim.new(0,12)
    ...
    fbMin.MouseEnter:Connect(function() AC.TS:Create(fbMin,TweenInfo.new(0.1),{BackgroundColor3=AC.PUR_DARK}):Play() end); fbMin.MouseLeave:Connect(function() AC.TS:Create(fbMin,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(40,40,40)}):Play() end)
    fbMin.MouseButton1Click:Connect(function() fbPanel.Visible=false end)
    local fbDrag,fbDS,fbSP=false,nil,nil
    fbHdr.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then fbDrag=true; fbDS=i.Position; fbSP=fbPanel.Position; i.Changed:Connect(function() if i.UserInputState==Enum.UserInputState.End then fbDrag=false end end) end end)
    AC.UIS.InputChanged:Connect(function(i) if fbDrag and i.UserInputType==Enum.UserInputType.MouseMovement then local d=i.Position-fbDS; local vp=AC.camera.ViewportSize; fbPanel.Position=UDim2.new(0,math.clamp(fbSP.X.Offset+d.X,0,vp.X-FB_W),0,math.clamp(fbSP.Y.Offset+d.Y,0,vp.Y-FB_H)) end end)
    ...
    fbBindBtn.MouseEnter:Connect(function() AC.TS:Create(fbBindBtn,TweenInfo.new(0.1),{BackgroundColor3=AC.PUR_DARK}):Play() end); fbBindBtn.MouseLeave:Connect(function() AC.TS:Create(fbBindBtn,TweenInfo.new(0.1),{BackgroundColor3=AC.BG_PANEL}):Play() end)
    local fbKey=nil; local fbBindListening=false; local fbBindConn=nil
    fbBindBtn.MouseButton1Click:Connect(function()
        if fbBindListening then fbBindListening=false; if fbBindConn then fbBindConn:Disconnect(); fbBindConn=nil end; fbBindBtn.Text=fbKey and "["..fbKey:sub(1,4).."]" or "Bind"; fbBindBtn.TextColor3=fbKey and AC.TXT_WHITE or AC.PUR_GLOW; return end
        fbBindListening=true; fbBindBtn.Text="..."; fbBindBtn.TextColor3=AC.ORANGE_W
        fbBindConn=AC.UIS.InputBegan:Connect(function(inp,gp)
            if gp or inp.UserInputType~=Enum.UserInputType.Keyboard then return end
            local ok2,kn=pcall(function() return inp.KeyCode.Name end); if not ok2 or not kn or kn=="" or kn=="Unknown" then return end
            if kn=="Escape" then fbBindListening=false; if fbBindConn then fbBindConn:Disconnect(); fbBindConn=nil end; fbBindBtn.Text=fbKey and "["..fbKey:sub(1,4).."]" or "Bind"; fbBindBtn.TextColor3=fbKey and AC.TXT_WHITE or AC.PUR_GLOW; return end
            fbKey=kn; fbBindBtn.Text="["..kn:sub(1,4).."]"; fbBindBtn.TextColor3=AC.TXT_WHITE; fbBindListening=false; if fbBindConn then fbBindConn:Disconnect(); fbBindConn=nil end; AC.toast("FaceBang bound to ["..kn.."]",AC.PUR_BRIGHT)
        end)
    end)
    ...
    local function fbGetNearestPlayer()
        local myChar=AC.player.Character; if not myChar then return nil end; local myRoot=myChar:FindFirstChild("HumanoidRootPart"); if not myRoot then return nil end
        local best=nil; local bestDist=math.huge
        for _,p in ipairs(AC.Players:GetPlayers()) do if p~=AC.player and p.Character then local r=p.Character:FindFirstChild("HumanoidRootPart"); if r then local d=(r.Position-myRoot.Position).Magnitude; if d<bestDist then bestDist=d; best=p end end end end
        return best
    end
    local function fbStop()
        if fbConn then fbConn:Disconnect(); fbConn=nil end; fbActive=false; fbLastTarget=nil
        local myChar=AC.player.Character
        if myChar then
            local mr=myChar:FindFirstChild("HumanoidRootPart"); local hum=myChar:FindFirstChildOfClass("Humanoid")
            if mr then pcall(function() mr.AssemblyLinearVelocity=Vector3.zero end); pcall(function() mr.AssemblyAngularVelocity=Vector3.zero end); local p=mr.CFrame.Position; mr.CFrame=CFrame.new(p)*CFrame.Angles(0,select(2,mr.CFrame:ToEulerAnglesYXZ()),0) end
            if hum then hum.PlatformStand=false; pcall(function() hum:ChangeState(Enum.HumanoidStateType.Running) end) end
            task.delay(0.1,function() pcall(function() if myChar and myChar.Parent then local h=myChar:FindFirstChildOfClass("Humanoid"); if h then h.PlatformStand=false; pcall(function() h:ChangeState(Enum.HumanoidStateType.Running) end) end; local r=myChar:FindFirstChild("HumanoidRootPart"); if r then pcall(function() r.AssemblyLinearVelocity=Vector3.zero end) end end end) end)
        end
        AC.TS:Create(fbTrk,TweenInfo.new(0.2),{BackgroundColor3=Color3.fromRGB(40,40,40)}):Play(); AC.TS:Create(fbKnob,TweenInfo.new(0.2),{Position=UDim2.new(0,3,0.5,-9),BackgroundColor3=AC.TXT_DIM}):Play()
        fbStatus.Text="Target: none  |  Status: OFF"; AC.toast("FaceBang OFF",AC.ORANGE_W)
    end
    local function fbStart()
        ...
        local myChar=AC.player.Character; if not myChar then return end; local myRoot=myChar:FindFirstChild("HumanoidRootPart"); if not myRoot then return end
        ...
        AC.TS:Create(fbTrk,TweenInfo.new(0.2),{BackgroundColor3=AC.PUR_MID}):Play(); AC.TS:Create(fbKnob,TweenInfo.new(0.2),{Position=UDim2.new(1,-21,0.5,-9),BackgroundColor3=AC.PUR_GLOW}):Play()
        fbStatus.Text="Target: "..tgt.Name.."  |  Status: ON"; AC.toast("FaceBang ON  ->  "..tgt.Name,AC.PUR_BRIGHT)
        local t=0
        fbConn=AC.RS.Heartbeat:Connect(function(dt)
            if not fbActive then return end; if not fbLastTarget or not fbLastTarget.Character then fbStop(); return end
            local th=fbLastTarget.Character:FindFirstChild("Head"); local mr=myChar:FindFirstChild("HumanoidRootPart"); if not th or not mr then fbStop(); return end
            t=t+dt*fbSpd; local osc=math.abs(math.sin(t)); local pull=math.clamp(osc*fbDist,0,fbDist)
            local headPos=th.CFrame.Position; local faceFwd=th.CFrame.LookVector; local targetPos=headPos+faceFwd*(1.8+pull)
            mr.CFrame=CFrame.new(targetPos,headPos)
        end)
    end
    fbTrk.MouseButton1Click:Connect(function() pcall(function() AC.clickSnd:Play() end); if fbActive then fbStop() else fbStart() end end)
    AC.UIS.InputBegan:Connect(function(inp,gp) pcall(function() if gp or inp.UserInputType~=Enum.UserInputType.Keyboard then return end; if not fbKey then return end; local kn=inp.KeyCode.Name; if not kn or kn=="" or kn=="Unknown" then return end; if kn==fbKey then if fbActive then fbStop() else fbStart() end end end) end)
    fbBtn.MouseButton1Click:Connect(function()
        fbPanel.Visible=not fbPanel.Visible
        if fbPanel.Visible then local vp=AC.camera.ViewportSize; local pos=fbPanel.Position; fbPanel.Position=UDim2.new(0,math.clamp(pos.X.Offset,0,vp.X-FB_W),0,math.clamp(pos.Y.Offset,0,vp.Y-FB_H)) end
    end)
end
...
do
    ...
    task.defer(function() local vp=AC.camera.ViewportSize; qBar.Position=UDim2.new(0,10,0,vp.Y-130) end)
    local qDrag=false; local qDS=nil; local qSPx=0; local qSPy=0
    qBar.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then qDrag=true; qDS=i.Position; qSPx=qBar.Position.X.Offset; qSPy=qBar.Position.Y.Offset; i.Changed:Connect(function() if i.UserInputState==Enum.UserInputState.End then qDrag=false end end) end end)
    AC.UIS.InputChanged:Connect(function(i) if qDrag and i.UserInputType==Enum.UserInputType.MouseMovement then local d=i.Position-qDS; local vp=AC.camera.ViewportSize; qBar.Position=UDim2.new(0,math.clamp(qSPx+d.X,0,vp.X-186),0,math.clamp(qSPy+d.Y,0,vp.Y-44)) end end)
...
do
    AC.switchTab("Home")
    AC.wrapper.Visible=false; AC.wrapper.Size=UDim2.new(0,0,0,0); AC.wrapper.Position=UDim2.new(0.5,0,0.5,0)
    task.delay(0.08,function()
        AC.wrapper.Visible=true; pcall(function() AC.openSound:Play() end)
        AC.TS:Create(AC.wrapper,TweenInfo.new(0.45,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Size=UDim2.new(0,AC.UI_W,0,AC.UI_H),Position=UDim2.new(0.5,-AC.UI_W/2,0.5,-AC.UI_H/2)}):Play()
        task.delay(0.5,function() AC._uiOpen=true end)
    end)
    task.delay(1.4,function() AC.toast("AC AudioCrafter v4.80 loaded!  G = toggle") end)
    print("AC AudioCrafter V4.80  by MelodyCrafter")
    ...
    pcall(function()
        AC.player.OnTeleport:Connect(function(teleportState)
            if teleportState==Enum.TeleportState.InProgress then
                pcall(function() if writefile and isfolder then writefile("AC/__autorejoin.json",game:GetService("HttpService"):JSONEncode({placeId=game.PlaceId,ver=AC_VER,time=os.time()})) end end)
                AC.toast("Teleporting — AC will reload on join",AC.PUR_BRIGHT)
            end
        end)
    end)
    pcall(function()
        if not isfile then return end; if not isfile("AC/__autorejoin.json") then return end
        local ok,data=pcall(function() return game:GetService("HttpService"):JSONDecode(readfile("AC/__autorejoin.json")) end)
        if ok and data and type(data)=="table" then local age=os.time()-(data.time or 0); if age<30 then task.delay(2,function() AC.toast("Auto-reloaded after server change!",AC.GREEN_OK) end) end end
