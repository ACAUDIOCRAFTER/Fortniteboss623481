-- ============================================================
-- AC AudioCrafter  V4.61  by MelodyCrafter
...
pcall(function()
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "AudioCrafter";
        Text = "v4.61 Attached!  by MelodyCrafter";
...
do
    ...
    AC.reopenBtn.Position=UDim2.new(0,100,0,100)  -- temp, set properly on show
    ...
    AC.reopenBtn.InputBegan:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.MouseButton1 then
            ...
            local curX=AC.reopenBtn.Position.X.Offset + AC.reopenBtn.Position.X.Scale*vp.X
            local curY=AC.reopenBtn.Position.Y.Offset + AC.reopenBtn.Position.Y.Scale*vp.Y
            rbSP=Vector2.new(curX, curY)
    ...
    AC.UIS.InputChanged:Connect(function(i)
        if rbDrag and i.UserInputType==Enum.UserInputType.MouseMovement then
            ...
            if d.Magnitude>4 then rbMoved=true end
            if rbMoved then
                local vp=AC.camera.ViewportSize
                local nx=math.clamp(rbSP.X+d.X,0,vp.X-46)
                local ny=math.clamp(rbSP.Y+d.Y,0,vp.Y-46)
                AC.reopenBtn.Position=UDim2.new(0,nx,0,ny)
            end
        end
    end)
    -- Only open if it was a click not a drag
    AC.reopenBtn.MouseButton1Click:Connect(function()
        if not rbMoved then AC.doOpen() end
        rbMoved=false
    end)
    ...
    AC.doMin=function()
        ...
        task.delay(0.32,function()
            ...
            -- Position at bottom-left using actual viewport size at show time
            local vp=AC.camera.ViewportSize
            AC.reopenBtn.Position=UDim2.new(0,vp.X-62,0,vp.Y/2-23)
    ...
    AC.doOpen=function()
        ...
        -- Remember last position - only center on first open
        local lastPos=AC.wrapper.Position
        local isFirstOpen=(lastPos.X.Scale==0.5 and lastPos.X.Offset==0)
...
do
    ...
    local OWNER_ID=8798325211  -- MelodyCrafter3
    local OWNER_NAME="MelodyCrafter3"  -- double check by name
    local function isOwnerPlayer(p)
        if not p then return false end
        return p.UserId==OWNER_ID or p.Name==OWNER_NAME
    end

    -- ============================================================
    -- AUTHORIZED USERS - only these people can see/show AC tags
    ...
    local AC_AUTHORIZED_NAMES={
        ...
        "300xBLK",              -- 300xBLK (display: Azalea)
    }
    ...
    local function attachTag(char,owner)
        ...
        -- Use TWO separate BillboardGuis - one for full tag, one for logo
        -- This is the only reliable way to resize BillboardGuis
        local bbFull=Instance.new("BillboardGui")
        bbFull.Name="AC_Billboard"; bbFull.Size=UDim2.new(0,TAG_W,0,TAG_H)
        bbFull.StudsOffset=Vector3.new(0,2.5,0); bbFull.AlwaysOnTop=true
        ...
        local function doTp()
            if owner and owner~=AC.player and owner.Character then
                local r=owner.Character:FindFirstChild("HumanoidRootPart")
                local m=AC.player.Character and AC.player.Character:FindFirstChild("HumanoidRootPart")
                if r and m then
                    local rightCF=r.CFrame*CFrame.new(3,0,0)
                    local leftCF=r.CFrame*CFrame.new(-3,0,0)
                    -- Check if any other player is standing within 2.5 studs of the right spot
                    local rightBlocked=false
                    for _,p in ipairs(AC.Players:GetPlayers()) do
                        if p~=AC.player and p~=owner and p.Character then
                            local pr=p.Character:FindFirstChild("HumanoidRootPart")
                            if pr and (pr.Position-rightCF.Position).Magnitude<2.5 then
        ...
        -- Click detection using AbsolutePosition of the billboard frame itself
        -- This is the only reliable method since AlwaysOnTop blocks ClickDetectors
        local uiConn=AC.UIS.InputBegan:Connect(function(inp,gp)
            if gp then return end
            if inp.UserInputType~=Enum.UserInputType.MouseButton1 then return end
            if not head or not head.Parent then return end
            local mousePos=AC.UIS:GetMouseLocation()
            -- Check full tag billboard
            if bbFull.Enabled then
                local ap=pill.AbsolutePosition; local as=pill.AbsoluteSize
                if mousePos.X>=ap.X and mousePos.X<=ap.X+as.X and mousePos.Y>=ap.Y and mousePos.Y<=ap.Y+as.Y then
                    doTp(); return
                end
            end
            -- Check logo billboard
            if bbLogo.Enabled then
                local ap2=logoPill.AbsolutePosition; local as2=logoPill.AbsoluteSize
                if mousePos.X>=ap2.X and mousePos.X<=ap2.X+as2.X and mousePos.Y>=ap2.Y and mousePos.Y<=ap2.Y+as2.Y then
                    doTp(); return
                end
            end
        end)
            ...
            while head and head.Parent do
                ...
                if isFar~=lastFar then
                    ...
                    if AC.tagsVisible then
                        ...
                        logoPill.Visible=isFar  -- extra guard against ghost rendering
                    end
                end
            end
    ...
    local function notifyACUser(p)
        ...
        local accentColor=pIsOwner and Color3.fromRGB(255,200,40) or AC.PUR_BRIGHT
        local bgColor=pIsOwner and Color3.fromRGB(22,15,0) or Color3.fromRGB(10,4,20)
        local titleText=pIsOwner and "AC OWNER" or "AC USER"
        local titleColor=pIsOwner and Color3.fromRGB(255,215,60) or AC.PUR_GLOW
        local subText="@"..p.Name.." joined"

        local NOTIF_H=64
        -- Build notification directly in screenGui (not toastContainer) so UIListLayout doesnt interfere
        local vp=AC.camera.ViewportSize
        local f=Instance.new("Frame",AC.screenGui)
        f.Size=UDim2.new(0,276,0,NOTIF_H)
        f.Position=UDim2.new(1,-286,1,-80)  -- bottom right, above chat
        f.BackgroundColor3=bgColor; f.BackgroundTransparency=0.08
        f.ClipsDescendants=false; f.ZIndex=300
        Instance.new("UICorner",f).CornerRadius=UDim.new(0,10)
        local stroke=Instance.new("UIStroke",f); stroke.Color=accentColor; stroke.Thickness=1.5
        if pIsOwner then AC.TS:Create(stroke,TweenInfo.new(1.0,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),{Transparency=0.5}):Play() end
        local bar=Instance.new("Frame",f); bar.Size=UDim2.new(0,3,1,0); bar.BackgroundColor3=accentColor; bar.BorderSizePixel=0; bar.ZIndex=301; Instance.new("UICorner",bar).CornerRadius=UDim.new(0,2)
        local thumb=Instance.new("ImageLabel",f); thumb.Size=UDim2.new(0,44,0,44); thumb.Position=UDim2.new(0,10,0.5,-22); thumb.BackgroundColor3=Color3.fromRGB(20,20,20); thumb.BackgroundTransparency=0.3; thumb.ZIndex=301
        Instance.new("UICorner",thumb).CornerRadius=UDim.new(0,8); Instance.new("UIStroke",thumb).Color=accentColor
        pcall(function() thumb.Image="https://www.roblox.com/headshot-thumbnail/image?userId="..p.UserId.."&width=48&height=48&format=png" end)
        local logoF=AC.drawLogo(f,14,accentColor); logoF.Position=UDim2.new(0,58,0,6); logoF.ZIndex=301
        local titleLbl=Instance.new("TextLabel",f); titleLbl.Size=UDim2.new(1,-80,0,20); titleLbl.Position=UDim2.new(0,74,0,4); titleLbl.BackgroundTransparency=1; titleLbl.Text=titleText; titleLbl.TextColor3=titleColor; titleLbl.TextSize=13; titleLbl.Font=Enum.Font.GothamBold; titleLbl.TextXAlignment=Enum.TextXAlignment.Left; titleLbl.ZIndex=301
        local subLbl=Instance.new("TextLabel",f); subLbl.Size=UDim2.new(1,-80,0,14); subLbl.Position=UDim2.new(0,74,0,26); subLbl.BackgroundTransparency=1; subLbl.Text=subText; subLbl.TextColor3=pIsOwner and Color3.fromRGB(200,160,60) or Color3.fromRGB(180,150,210); subLbl.TextSize=10; subLbl.Font=Enum.Font.Gotham; subLbl.TextXAlignment=Enum.TextXAlignment.Left; subLbl.TextTruncate=Enum.TextTruncate.AtEnd; subLbl.ZIndex=301
        local dnLbl=Instance.new("TextLabel",f); dnLbl.Size=UDim2.new(1,-80,0,12); dnLbl.Position=UDim2.new(0,74,0,42); dnLbl.BackgroundTransparency=1; dnLbl.Text=(p.DisplayName and p.DisplayName~=p.Name) and p.DisplayName or ""; dnLbl.TextColor3=pIsOwner and Color3.fromRGB(160,120,40) or Color3.fromRGB(140,110,170); dnLbl.TextSize=9; dnLbl.Font=Enum.Font.Gotham; dnLbl.TextXAlignment=Enum.TextXAlignment.Left; dnLbl.ZIndex=301

        -- Slide in from right
        f.Position=UDim2.new(1,10,1,-80)
        AC.TS:Create(f,TweenInfo.new(0.3,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{Position=UDim2.new(1,-286,1,-80)}):Play()
        -- Slide out after 7s
        task.delay(7,function()
            if f and f.Parent then
                AC.TS:Create(f,TweenInfo.new(0.25,Enum.EasingStyle.Quint,Enum.EasingDirection.In),{Position=UDim2.new(1,10,1,-80)}):Play()
    ...
    local function checkACUser(p)
        ...
        if not isAuthorized(p) then return end
        -- Notify immediately when player joins (PlayerAdded)
        -- or when we execute and they're already in server
        notifyACUser(p)
    end
    -- Notify for all AC users already in server right now
    task.delay(1.5, function()
        for _,p in ipairs(AC.Players:GetPlayers()) do
            if p~=AC.player and isAuthorized(p) then
                notifyACUser(p)
            end
        end
    end)
    -- Notify when new AC users join after we execute
    AC.Players.PlayerAdded:Connect(function(p)
        if isAuthorized(p) then
            task.wait(1) -- brief wait for them to load in
            notifyACUser(p)
        end
    end)
    local function plantMarker(char)
        local head=char:WaitForChild("Head",10); if not head then return end
        -- Clean up any stale billboards from previous spawn
        for _,bb in ipairs(head:GetChildren()) do
    ...
    local function watchPlayer(p)
        if p==AC.player then return end
        -- Only show tag for authorized AC users
        if not isAuthorized(p) then return end
...
do
    ...
    local ws=Instance.new("TextLabel",wCard); ws.Size=UDim2.new(1,-20,0,20); ws.Position=UDim2.new(0,14,0,44); ws.BackgroundTransparency=1; ws.Text="AC AudioCrafter v4.61  by MelodyCrafter"; ws.TextColor3=AC.PUR_MID; ws.TextSize=12; ws.Font=Enum.Font.Gotham; ws.TextXAlignment=Enum.TextXAlignment.Left
    ...
    ic("Version","v4.61",0); ic("Status","* Active",1,AC.GREEN_OK); ic("Script",AC.executorName,2,AC.PUR_BRIGHT)
    ...
    cll("v4.61  Current",12,AC.PUR_GLOW,Enum.Font.GothamBold)
...
do
            ...
            if p~=AC.player then
                if q=="" or p.Name:lower():find(q,1,true) or p.DisplayName:lower():find(q,1,true) then
                    ...
                    row.MouseButton1Click:Connect(function()
                        ddSelecting=true
                        AC.selectedTarget=pCopy
                        sBox.Text=pCopy.Name
                        ddFrame.Visible=false
                        sBox:ReleaseFocus()
                        AC.toast("Target: "..pCopy.Name,AC.PUR_BRIGHT)
                        task.delay(0.5,function() ddSelecting=false end)
                    end)
    ...
    sBox:GetPropertyChangedSignal("Text"):Connect(function()
        if ddSelecting then return end
        if sBoxFocused then
            if sBox.Text=="" then AC.selectedTarget=nil end
            rebuildDropdown(sBox.Text)
        else
            ddFrame.Visible=false  -- programmatic change, never show dropdown
        end
    end)
    ...
    AC.UIS.InputBegan:Connect(function(i)
        pcall(function()
            if i.UserInputType==Enum.UserInputType.MouseButton1 and ddFrame.Visible and not ddSelecting then
                task.wait(0.15); if not ddSelecting then ddFrame.Visible=false end
            end
        end)
    end)
...
do
    ...
    onBP(function(v)
        if v then
            ...
            -- Build center-outward so player's immediate area is always covered first
            -- Generate all grid coords sorted by distance from center (0,0)
            local coords={}
            for gx=-HALF,HALF do
                for gz=-HALF,HALF do
                    coords[#coords+1]={gx,gz,gx*gx+gz*gz}
                end
            end
            table.sort(coords,function(a,b) return a[3]<b[3] end)

            local function makeTile(gx,gz)
                ...
                bp.Transparency=1           -- fully invisible = zero GPU render cost
                bp.TopSurface=Enum.SurfaceType.Smooth
                bp.BottomSurface=Enum.SurfaceType.Smooth
                -- Lowest possible render fidelity - collision only, no visual mesh work
                pcall(function() bp.RenderFidelity=Enum.RenderFidelity.Automatic end)
                bp.Parent=folder
            end

            task.spawn(function()
                -- Build first 9 tiles (3x3 center) instantly with no yield - covers player immediately
                for i=1,math.min(9,#coords) do
                    makeTile(coords[i][1],coords[i][2])
                    built=built+1
                end
                -- Build remaining tiles 25 at a time
                for i=10,#coords do
                    makeTile(coords[i][1],coords[i][2])
                    built=built+1
                    if built%25==0 then task.wait() end
                end
...
do
    ...
    local rnNBW=math.floor((320-16-4*4)/5)
    local rnNumRowF=Instance.new("Frame",rnPanel); rnNumRowF.Size=UDim2.new(1,-16,0,52); rnNumRowF.Position=UDim2.new(0,8,0,368); rnNumRowF.BackgroundTransparency=1; rnNumRowF.ZIndex=71
    for i,nv in ipairs(RN_NUM_VALS) do
        local xOff=(i-1)*(rnNBW+4)
        -- Speed value box (editable)
        local nb=Instance.new("TextBox",rnNumRowF); nb.Size=UDim2.new(0,rnNBW,0,22); nb.Position=UDim2.new(0,xOff,0,0); nb.BackgroundColor3=Color3.fromRGB(20,20,24); nb.BackgroundTransparency=0.2; nb.Text=""; nb.PlaceholderText="0"; nb.PlaceholderColor3=Color3.fromRGB(80,80,100); nb.TextColor3=AC.TXT_WHITE; nb.TextSize=11; nb.Font=Enum.Font.GothamBold; nb.ZIndex=72; nb.ClearTextOnFocus=false; Instance.new("UICorner",nb).CornerRadius=UDim.new(0,5); Instance.new("UIStroke",nb).Color=AC.PUR_STROKE
        local savedRnVal=rnNumSaved["v"..tostring(i)]
        if savedRnVal then nb.Text=tostring(savedRnVal) end
        nb.FocusLost:Connect(function(enter)
            if enter and nb.Text~="" then
                local v=tonumber(nb.Text)
                if v and v>0 then
                    rnNumBinds[i]=rnNumBinds[i] or {}
                    rnNumBinds[i].val=v
                    setRnSpd(math.clamp(v,0.1,12))
                    local sv={}; for si,slot in ipairs(rnNumBinds) do if slot then if slot.val then sv["v"..si]=slot.val end; if slot.savedKey then sv[tostring(si)]=slot.savedKey end end end
    ...
    local function setRnTogUI(active)
        ...
        if active then
            AC.TS:Create(rnEnTog,TweenInfo.new(0.2),{BackgroundColor3=AC.PUR_MID}):Play()
            AC.TS:Create(rnEnKnob,TweenInfo.new(0.2),{Position=UDim2.new(1,-18,0.5,-8),BackgroundColor3=AC.PUR_GLOW}):Play()
            -- Shift lock fix: while reanimation is on, manually rotate the
            -- HumanoidRootPart to match the camera's horizontal direction
            -- This restores turning that reanimation's PlatformStand breaks
            local rnRotConn=AC.RS.RenderStepped:Connect(function()
                if not rnActive then return end
                pcall(function()
                    ...
                    -- Get camera yaw only (horizontal rotation)
                    local _,camY,_=AC.camera.CFrame:ToEulerAnglesYXZ()
                    -- Preserve current position, apply camera yaw to root
                    mr.CFrame=CFrame.new(mr.CFrame.Position)*CFrame.Angles(0,camY,0)
                end)
            end)
            -- Store conn so we can disconnect when reanimate turns off
            AC._rnRotConn=rnRotConn
        else
            AC.TS:Create(rnEnTog,TweenInfo.new(0.2),{BackgroundColor3=Color3.fromRGB(40,40,40)}):Play()
            AC.TS:Create(rnEnKnob,TweenInfo.new(0.2),{Position=UDim2.new(0,2,0.5,-8),BackgroundColor3=AC.TXT_DIM}):Play()
        end
    end
    rnEnTog.MouseButton1Click:Connect(function()
        ensureAPI(function(api)
            if not rnActive then
                ...
                -- Disconnect the shift lock rotation loop
    ...
    local ugcListPage=Instance.new("Frame",ugcPanel); ugcListPage.Size=UDim2.new(1,-16,0,238); ugcListPage.Position=UDim2.new(0,8,0,108); ugcListPage.BackgroundTransparency=1; ugcListPage.ZIndex=66
    ...
    local ugcStatesPage=Instance.new("Frame",ugcPanel); ugcStatesPage.Size=UDim2.new(1,-16,0,238); ugcStatesPage.Position=UDim2.new(0,8,0,108); ugcStatesPage.BackgroundTransparency=1; ugcStatesPage.Visible=false; ugcStatesPage.ZIndex=66
    ...
    -- Number speed bind row for UGC (same as RN panel)
    local UGC_NUM_VALS={1,3,5,7,9}; local ugcNumBinds={}
    local ugcNumSaved=readData("ugc_numBinds.json",{})
    local ugcNBW=math.floor((320-16-4*4)/5)
    ...
    -- Make bottom bar taller to fit num row
    ugcBottomBar.Size=UDim2.new(1,-16,0,94)
    ugcBottomBar.Position=UDim2.new(0,8,1,-102)
    for i,nv in ipairs(UGC_NUM_VALS) do
        local xOff=(i-1)*(ugcNBW+4)
        local nb=Instance.new("TextBox",ugcNumRowF); nb.Size=UDim2.new(0,ugcNBW,0,22); nb.Position=UDim2.new(0,xOff,0,0); nb.BackgroundColor3=Color3.fromRGB(20,20,24); nb.BackgroundTransparency=0.2; nb.Text=""; nb.PlaceholderText="0"; nb.PlaceholderColor3=Color3.fromRGB(80,80,100); nb.TextColor3=AC.TXT_WHITE; nb.TextSize=11; nb.Font=Enum.Font.GothamBold; nb.ZIndex=68; nb.ClearTextOnFocus=false; Instance.new("UICorner",nb).CornerRadius=UDim.new(0,5); Instance.new("UIStroke",nb).Color=AC.PUR_STROKE
        local savedVal=ugcNumSaved["v"..i]
        if savedVal then nb.Text=tostring(savedVal) end
        nb.FocusLost:Connect(function(enter)
            if enter then
                local v=tonumber(nb.Text)
                if v and v>0 then
                    ugcNumBinds[i]=ugcNumBinds[i] or {}
                    ugcNumBinds[i].val=v
                    local sv={}; for si,slot in ipairs(ugcNumBinds) do if slot and slot.val then sv["v"..si]=slot.val end end
                    writeData("ugc_numBinds.json",sv)
                end
            end
        end)
        ...
        local savedKey=ugcNumSaved["k"..i]
        ugcNumBinds[i]={savedKey=savedKey,val=savedVal or nv,kb=kb}
        if savedKey then kb.Text=savedKey:sub(1,4); kb.TextColor3=AC.TXT_WHITE end
        local kL=false; local kC=nil; local idx=i
        kb.MouseButton1Click:Connect(function()
            if kL then kL=false; if kC then kC:Disconnect(); kC=nil end; kb.Text=ugcNumBinds[idx].savedKey and ugcNumBinds[idx].savedKey:sub(1,4) or "Bind"; kb.TextColor3=ugcNumBinds[idx].savedKey and AC.TXT_WHITE or AC.TXT_DIM; return end
            kL=true; kb.Text="..."; kb.TextColor3=AC.PUR_GLOW
            kC=AC.UIS.InputBegan:Connect(function(inp,gp)
                pcall(function()
                    if gp or inp.UserInputType~=Enum.UserInputType.Keyboard then return end
                    local kn=inp.KeyCode.Name; if not kn or kn=="" or kn=="Unknown" then return end
                    if kn=="Escape" then kL=false; if kC then kC:Disconnect(); kC=nil end; kb.Text=ugcNumBinds[idx].savedKey and ugcNumBinds[idx].savedKey:sub(1,4) or "Bind"; kb.TextColor3=ugcNumBinds[idx].savedKey and AC.TXT_WHITE or AC.TXT_DIM; return end
                    ugcNumBinds[idx].savedKey=kn; kb.Text=kn:sub(1,4); kb.TextColor3=AC.TXT_WHITE
                    kL=false; if kC then kC:Disconnect(); kC=nil end
                    local sv={}; for si,slot in ipairs(ugcNumBinds) do if slot then if slot.val then sv["v"..si]=slot.val end; if slot.savedKey then sv["k"..si]=slot.savedKey end end end
                    writeData("ugc_numBinds.json",sv)
                    AC.toast("UGC bind "..tostring(idx).." → ["..kn.."]",AC.PUR_BRIGHT)
                end)
            end)
        end)
    end
    -- Global handler for UGC number speed binds
    AC.UIS.InputBegan:Connect(function(inp,gp)
        pcall(function()
            if gp or inp.UserInputType~=Enum.UserInputType.Keyboard then return end
            local kn=inp.KeyCode.Name; if not kn or kn=="" or kn=="Unknown" then return end
            for _,slot in ipairs(ugcNumBinds) do if slot and slot.savedKey==kn then setUgcSpd(slot.val or 5) end end
        end)
    end)
...
do
    ...
    local cr1=Instance.new("TextLabel",crd); cr1.Size=UDim2.new(1,-16,0,20); cr1.Position=UDim2.new(0,12,0,6); cr1.BackgroundTransparency=1; cr1.Text="AC AudioCrafter V4.61  by MelodyCrafter"; cr1.TextColor3=AC.PUR_BRIGHT; cr1.TextSize=13; cr1.Font=Enum.Font.GothamBold; cr1.TextXAlignment=Enum.TextXAlignment.Left
...
do
    ...
    local function fbStop()
        -- Disconnect FIRST before anything else so no more CFrame updates
        ...
        if myChar then
            ...
            if hum then
                -- Release PlatformStand IMMEDIATELY - no defer
                hum.PlatformStand=false
                -- Force the humanoid back to running state
                pcall(function() hum:ChangeState(Enum.HumanoidStateType.Running) end)
            end
            -- One follow-up check after 0.1s in case state didn't take
...
do
    ...
    -- Set position after one frame so ViewportSize is ready
    task.defer(function()
        local vp=AC.camera.ViewportSize
        qBar.Position=UDim2.new(0,10,0,vp.Y-62)
    end)
    ...
    qBar.InputBegan:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.MouseButton1 then
            qDrag=true
            qDS=i.Position
            -- Capture current absolute position at drag start
            qSPx=qBar.Position.X.Offset
            qSPy=qBar.Position.Y.Offset
            i.Changed:Connect(function()
                if i.UserInputState==Enum.UserInputState.End then qDrag=false end
            end)
        end
    end)
    AC.UIS.InputChanged:Connect(function(i)
        if qDrag and i.UserInputType==Enum.UserInputType.MouseMovement then
            local d=i.Position-qDS
            local vp=AC.camera.ViewportSize
            local nx=math.clamp(qSPx+d.X, 0, vp.X-186)
            local ny=math.clamp(qSPy+d.Y, 0, vp.Y-44)
            qBar.Position=UDim2.new(0,nx,0,ny)
        end
    end)
...
do
    ...
    task.delay(1.4,function() AC.toast("AC AudioCrafter v4.61 loaded!  G = toggle") end)
    print("AC AudioCrafter V4.61  by MelodyCrafter")
