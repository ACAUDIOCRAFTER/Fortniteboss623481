-- ============================================================
-- AC AudioCrafter  V4.2  by MelodyCrafter
...
pcall(function()
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "AudioCrafter";
        Text = "v4.2 Attached!  by MelodyCrafter";
...
AC.UI_W=720; AC.UI_H=500; AC.NAV_H=40; AC.SIDE_W=160; AC.MAIN_W=AC.UI_W-AC.SIDE_W-6
...
do
    ...
    AC.reopenBtn=Instance.new("TextButton",AC.screenGui); AC.reopenBtn.Size=UDim2.new(0,50,0,50); AC.reopenBtn.Position=UDim2.new(1,-62,0.5,-25); AC.reopenBtn.BackgroundColor3=AC.BG_PANEL; AC.reopenBtn.Text=""; AC.reopenBtn.Visible=false; AC.reopenBtn.ZIndex=20; Instance.new("UICorner",AC.reopenBtn).CornerRadius=UDim.new(1,0)
    ...
    -- FIX: doMin/doOpen only tween the wrapper. Inner panels NEVER resized (ClipsDescendants bug)
    AC.doMin=function()
...
do
    ...
    local ws=Instance.new("TextLabel",wCard); ws.Size=UDim2.new(1,-20,0,20); ws.Position=UDim2.new(0,14,0,44); ws.BackgroundTransparency=1; ws.Text="AC AudioCrafter v4.2  by MelodyCrafter"; ws.TextColor3=AC.PUR_MID; ws.TextSize=12; ws.Font=Enum.Font.Gotham; ws.TextXAlignment=Enum.TextXAlignment.Left
    ...
    ic("Version","v4.2",0); ic("Status","* Active",1,AC.GREEN_OK); ic("Script",AC.executorName,2,AC.PUR_BRIGHT)
    ...
    cll("v4.2  Current",12,AC.PUR_GLOW,Enum.Font.GothamBold)
...
do
    ...
    pcall(function() if isfile and isfile("BLEED/"..tostring(AC.player.UserId).."/toggle_key.json") then local d=AC.Http:JSONDecode(readfile("BLEED/"..tostring(AC.player.UserId).."/toggle_key.json")); if d and d.key then AC._toggleKey=d.key; kbValLbl.Text="["..d.key.."]" end end end)
    ...
    kbBtn.MouseButton1Click:Connect(function()
        ...
        _kbConn=AC.UIS.InputBegan:Connect(function(inp,gp)
            ...
            AC._toggleKey=kn; kbValLbl.Text="["..kn.."]"; kbValLbl.TextColor3=AC.GREEN_OK; kbTopLbl.Text="Toggle UI Key"; kbStr.Color=AC.PUR_STROKE; _kbListening=false; if _kbConn then _kbConn:Disconnect(); _kbConn=nil end
            pcall(function() if not writefile then return end; local dir="BLEED/"..tostring(AC.player.UserId); pcall(function() if not isfolder("BLEED") then makefolder("BLEED") end end); pcall(function() if not isfolder(dir) then makefolder(dir) end end); writefile(dir.."/toggle_key.json",AC.Http:JSONEncode({key=kn})) end)
    ...
    baBtn.MouseButton1Click:Connect(function()
        if AC.inBp then AC.inBp=false; if AC.bpConn then AC.bpConn:Disconnect(); AC.bpConn=nil end; local mh=AC.player.Character and AC.player.Character:FindFirstChildOfClass("Humanoid"); if mh then mh.Sit=false end; baBtn.Text="Back-to-Back"; AC.toast("Back-to-Back OFF",AC.ORANGE_W); return end
        if not AC.selectedTarget or not AC.selectedTarget.Character then AC.toast("No target",AC.RED_ERR); return end
        AC.inBp=true; baBtn.Text="Exit B2B"
        AC.bpConn=AC.RS.Heartbeat:Connect(function()
            if not AC.inBp then return end
            local mr=AC.player.Character and AC.player.Character:FindFirstChild("HumanoidRootPart"); local mh2=AC.player.Character and AC.player.Character:FindFirstChildOfClass("Humanoid"); local tr=AC.selectedTarget.Character and AC.selectedTarget.Character:FindFirstChild("HumanoidRootPart")
            if mr and tr then if mh2 then mh2.Sit=true end; mr.CFrame=tr.CFrame*CFrame.new(0,0.5,2) end
        end)
    end)
...
do
    ...
    AC.sectionLbl(pg,"PERFORMANCE",316)
    local _,_,onFU=AC.makeToggle(pg,"FPS Unlocker",334,false); onFU(function(v) if v then pcall(function() setfpscap(0) end) else pcall(function() setfpscap(60) end) end end)
    local _,_,onAFK=AC.makeToggle(pg,"Anti-AFK",380,false); onAFK(function(v) if v then local vu=game:GetService("VirtualUser"); AC.afkThread=task.spawn(function() while true do pcall(function() vu:Button2Down(Vector2.new(0,0),CFrame.new()); task.wait(0.1); vu:Button2Up(Vector2.new(0,0),CFrame.new()) end); task.wait(55) end end) else if AC.afkThread then task.cancel(AC.afkThread); AC.afkThread=nil end end end)
...
do
    ...
    rnEnTog.MouseButton1Click:Connect(function()
        ensureAPI(function(api)
            if not rnActive then
                ...
                else AC.toast("Reanimate failed",AC.RED_ERR) end
            else
                if rnNowUrl then pcall(function() api.stop_animation() end) end
                pcall(function() api.reanimate(false) end)
                local char=AC.player.Character; if char then local hum=char:FindFirstChildOfClass("Humanoid"); if hum then pcall(function() hum:ChangeState(Enum.HumanoidStateType.GettingUp) end); hum.PlatformStand=false end end
    ...
    local ugcPanel=Instance.new("Frame",AC.screenGui); ugcPanel.Size=UDim2.new(0,UGC_W,0,UGC_H); ugcPanel.Position=UDim2.new(0.5,-UGC_W/2,0.15,0); ugcPanel.BackgroundColor3=Color3.fromRGB(12,12,14); ugcPanel.BackgroundTransparency=0.18; ugcPanel.ZIndex=65; ugcPanel.Visible=false; ugcPanel.ClipsDescendants=true; Instance.new("UICorner",ugcPanel).CornerRadius=UDim.new(0,10); Instance.new("UIStroke",ugcPanel).Color=AC.PUR_STROKE
    ...
    local function stopUgcTrack()
        if ugcActiveTrack then pcall(function() ugcActiveTrack:Stop(0); ugcActiveTrack:Destroy() end); ugcActiveTrack=nil end
        for _,t in ipairs(ugcLoadedTracks) do pcall(function() if t then t:Stop(0); t:Destroy() end end) end; ugcLoadedTracks={}
        pcall(function() local char=AC.player.Character; if not char then return end; local hum=char:FindFirstChildOfClass("Humanoid"); if not hum then return end; local animator=hum:FindFirstChildOfClass("Animator"); if animator then for _,t in ipairs(animator:GetPlayingAnimationTracks()) do pcall(function() t:Stop(0) end) end end; task.defer(function() pcall(function() hum:ChangeState(Enum.HumanoidStateType.GettingUp) end) end) end)
    end
    local function playUgcEmote(id)
        ...
        if ok2 and t2 then for _,existing in ipairs(animator:GetPlayingAnimationTracks()) do pcall(function() existing:Stop(0); existing:Destroy() end) end; t2.Priority=Enum.AnimationPriority.Action4; t2.Looped=true; t2:Play(0); pcall(function() t2:AdjustSpeed(ugcSpdCur) end); ugcActiveTrack=t2 end
    end
    local function buildUgcRow(item,idx)
        ...
        star.MouseButton1Click:Connect(function() if ugcFavs[idxStr] then ugcFavs[idxStr]=nil; star.Text="[ ]"; star.TextColor3=AC.TXT_DIM else ugcFavs[idxStr]=true; star.Text="[*]"; AC.TS:Create(star,TweenInfo.new(0.15),{TextColor3=Color3.fromRGB(255,210,60)}):Play() end; saveUgcFavs(); if ugcFavOnly then row.Visible=ugcFavs[idxStr]~=nil end end)
        ...
        bLbl.MouseButton1Click:Connect(function()
            ...
            bC=AC.UIS.InputBegan:Connect(function(inp,gp) if gp or inp.UserInputType~=Enum.UserInputType.Keyboard then return end; local ok2,kn=pcall(function() return inp.KeyCode.Name end); if not ok2 or not kn or kn=="" or kn=="Unknown" then return end; ugcBinds[idxStr]=kn; saveUgcBinds(); bLbl.Text=kn:sub(1,3); bLbl.TextColor3=AC.TXT_WHITE; bL=false; if bC then bC:Disconnect(); bC=nil end end)
        end)
        local highlighted=false
        row.MouseButton1Click:Connect(function()
            if highlighted then stopUgcTrack(); highlighted=false; if ugcHighRow==row then ugcHighRow=nil end; AC.TS:Create(row,TweenInfo.new(0.15),{BackgroundColor3=Color3.fromRGB(14,14,18)}):Play(); rS.Color=AC.PUR_STROKE; AC.toast("Stopped",AC.ORANGE_W); return end
            if ugcHighRow and ugcHighRow~=row then pcall(function() AC.TS:Create(ugcHighRow,TweenInfo.new(0.15),{BackgroundColor3=Color3.fromRGB(14,14,18)}):Play(); local os=ugcHighRow:FindFirstChildOfClass("UIStroke"); if os then os.Color=AC.PUR_STROKE end end); ugcHighRow=nil end
            stopUgcTrack(); playUgcEmote(item.id); highlighted=true; ugcHighRow=row
            AC.TS:Create(row,TweenInfo.new(0.15),{BackgroundColor3=AC.PUR_DARK}):Play(); rS.Color=AC.PUR_BRIGHT; AC.toast("* "..item.name,AC.PUR_BRIGHT)
        end)
        row.MouseEnter:Connect(function() if not highlighted then AC.TS:Create(row,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(22,22,28)}):Play() end end)
        row.MouseLeave:Connect(function() if not highlighted then AC.TS:Create(row,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(14,14,18)}):Play() end end)
    ...
    local function ugcSwitchTab(tabName)
        ...
        if tabName~="States" then ugcFavOnly=(tabName=="Favs"); local q=ugcSrch.Text:lower(); for _,r in ipairs(ugcAllRows) do if r then local sf=(not ugcFavOnly) or ugcFavs[tostring(r.id)]; r.row.Visible=sf and (q=="" or r.nameLower:find(q,1,true)~=nil) end end end
    end
    for tn,tb in pairs(ugcTabBtns) do local n=tn; tb.MouseButton1Click:Connect(function() ugcSwitchTab(n) end) end
    ugcSrch:GetPropertyChangedSignal("Text"):Connect(function() if ugcActiveTab=="States" then return end; ugcFavOnly=(ugcActiveTab=="Favs"); local q=ugcSrch.Text:lower(); for _,r in ipairs(ugcAllRows) do if r then local sf=(not ugcFavOnly) or ugcFavs[tostring(r.id)]; r.row.Visible=sf and (q=="" or r.nameLower:find(q,1,true)~=nil) end end end)
    AC.UIS.InputBegan:Connect(function(inp,gp)
        pcall(function()
            if gp or inp.UserInputType~=Enum.UserInputType.Keyboard then return end
            local kn=inp.KeyCode.Name; if not kn or kn=="" or kn=="Unknown" then return end
    ...
    local function loadUgcData()
        ...
        task.spawn(function()
            ...
            for i=1,#emotes do local built=buildUgcRow(emotes[i],i); ugcAllRows[i]=built; if built and ugcFavOnly then built.row.Visible=(ugcFavs[tostring(emotes[i].id)]==true) end; if i%30==0 then ugcScroll.CanvasSize=UDim2.new(0,0,0,i*36+8); task.wait(0.05) end end
            ugcScroll.CanvasSize=UDim2.new(0,0,0,#emotes*36+8); if #emotes==0 then ugcStatusLbl.Text="No emotes found" end
        end)
    end
    ...
    rnMainBtn.MouseButton1Click:Connect(function() pcall(function() AC.clickSnd:Play() end); rnPanel.Visible=not rnPanel.Visible; if rnPanel.Visible then local vp=AC.camera.ViewportSize; local pos=rnPanel.Position; if pos.X.Offset<0 or pos.X.Offset>vp.X-50 or pos.Y.Offset<0 or pos.Y.Offset>vp.Y-50 then rnPanel.Position=UDim2.new(0.5,-RN_W/2,0.5,-RN_H/2) end end end)
    ...
    ugcMainBtn.MouseButton1Click:Connect(function() pcall(function() AC.clickSnd:Play() end); ugcPanel.Visible=not ugcPanel.Visible; if ugcPanel.Visible then local vp=AC.camera.ViewportSize; local pos=ugcPanel.Position; if pos.X.Offset<0 or pos.X.Offset>vp.X-50 or pos.Y.Offset<0 or pos.Y.Offset>vp.Y-50 then ugcPanel.Position=UDim2.new(0.5,-UGC_W/2,0.5,-UGC_H/2) end; loadUgcData() end end)
...
do
    ...
    local ctConn=nil
    local _,_,onCTP=AC.makeToggle(pg,"Click Teleport",28,false)
    ...
    local _,_,onAV=AC.makeToggle(pg,"Anti-Void",74,false)
    ...
    local vcOuter=Instance.new("Frame",pg); vcOuter.Size=UDim2.new(1,-24,0,62); vcOuter.Position=UDim2.new(0,12,0,242); vcOuter.BackgroundTransparency=1
    ...
    local cr1=Instance.new("TextLabel",crd); cr1.Size=UDim2.new(1,-16,0,20); cr1.Position=UDim2.new(0,12,0,6); cr1.BackgroundTransparency=1; cr1.Text="AC AudioCrafter V4.2  by MelodyCrafter"; cr1.TextColor3=AC.PUR_BRIGHT; cr1.TextSize=13; cr1.Font=Enum.Font.GothamBold; cr1.TextXAlignment=Enum.TextXAlignment.Left
...
do
    ...
    local fbBtn=AC.createTab("FacBang",94,true)
    if AC.tabs["FacBang"] then
        if AC.tabs["FacBang"].dot then AC.tabs["FacBang"].dot.BackgroundColor3=AC.PUR_BRIGHT end
        if AC.tabs["FacBang"].lbl then AC.tabs["FacBang"].lbl.TextColor3=AC.PUR_GLOW end
        if AC.tabs["FacBang"].acc then AC.tabs["FacBang"].acc.BackgroundColor3=AC.PUR_BRIGHT end
    end

    local FB_W,FB_H=280,258
    local fbPanel=Instance.new("Frame",AC.screenGui)
    fbPanel.Size=UDim2.new(0,FB_W,0,FB_H)
    fbPanel.Position=UDim2.new(0.5,-FB_W/2,0.5,-FB_H/2)
    ...
    local fbTitle=Instance.new("TextLabel",fbHdr); fbTitle.Size=UDim2.new(1,-50,1,0); fbTitle.Position=UDim2.new(0,14,0,0); fbTitle.BackgroundTransparency=1; fbTitle.Text="★  FacBang"; fbTitle.TextColor3=AC.TXT_WHITE; fbTitle.TextSize=15; fbTitle.Font=Enum.Font.GothamBold; fbTitle.TextXAlignment=Enum.TextXAlignment.Left; fbTitle.ZIndex=82
    ...
    fbBindBtn.MouseButton1Click:Connect(function()
        ...
        fbBindConn=AC.UIS.InputBegan:Connect(function(inp,gp)
            ...
            fbBindListening=false; if fbBindConn then fbBindConn:Disconnect(); fbBindConn=nil end
            AC.toast("FacBang bound to ["..kn.."]",AC.PUR_BRIGHT)
        end)
    end)
    ...
    local fbActiveLbl=Instance.new("TextLabel",fbActiveBg); fbActiveLbl.Size=UDim2.new(1,-60,1,0); fbActiveLbl.Position=UDim2.new(0,12,0,0); fbActiveLbl.BackgroundTransparency=1; fbActiveLbl.Text="FacBang Active"; fbActiveLbl.TextColor3=AC.TXT_MAIN; fbActiveLbl.TextSize=13; fbActiveLbl.Font=Enum.Font.GothamBold; fbActiveLbl.TextXAlignment=Enum.TextXAlignment.Left; fbActiveLbl.ZIndex=82
    ...
    local function fbStop()
        ...
        local myChar=AC.player.Character; if myChar then local hum=myChar:FindFirstChildOfClass("Humanoid"); if hum then hum.PlatformStand=false end end
        ...
        AC.toast("FacBang OFF",AC.ORANGE_W)
    end
    local function fbStart()
        ...
        AC.toast("FacBang ON  ->  "..tgt.Name,AC.PUR_BRIGHT)
        local t=0
    ...
    fbBtn.MouseButton1Click:Connect(function()
        ...
        if fbPanel.Visible then
            ...
            if pos.X.Offset~=cx or pos.Y.Offset~=cy then
...
do
    ...
    task.delay(1.4,function() AC.toast("AC AudioCrafter v4.2 loaded!  G = toggle") end)
    print("AC AudioCrafter V4.2  by MelodyCrafter")
