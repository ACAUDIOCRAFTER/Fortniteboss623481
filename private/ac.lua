...
-- API endpoints (reuse yourscoper vercel - same as AKLibrary)
local AC_NAMETAG_URL = "https:".._sl.."yourscoper.vercel.app/api/nametag"
local AC_TAG_SECRET  = ""  -- leave blank, server handles auth via AC_SECRET env

-- Registry: userId(string) -> {tag=string, executed=bool}
local AC_DatabaseCache = {}
local AC_TaggedPlayers = {}

local nameTagsEnabled = true

-- API helpers
local function AC_RegisterSelf()
    if not Request then return end
    local info = SPECIAL[lp.Name]
    local tag = info and info.tag or "AC USER"
    pcall(function()
        Request({
            Url = AC_NAMETAG_URL .. "?secret=" .. AC_TAG_SECRET,
            Method = "POST",
            Headers = {["x-secret"]=AC_TAG_SECRET, ["Content-Type"]="application/json"},
            Body = HttpService:JSONEncode({
...
local function AC_SetExecuted(val)
    if not Request then return end
    pcall(function()
        Request({
            Url = AC_NAMETAG_URL .. "?secret=" .. AC_TAG_SECRET,
            Method = "POST",
            Headers = {["x-secret"]=AC_TAG_SECRET, ["Content-Type"]="application/json"},
            Body = HttpService:JSONEncode({userId=tostring(lp.UserId), executed=val})
        })
    end)
end

local function AC_FetchAllTags()
    if not Request then return nil end
    local ok, result = pcall(function()
        return Request({
            Url = AC_NAMETAG_URL .. "?secret=" .. AC_TAG_SECRET,
            Method = "GET",
            Headers = {["x-secret"]=AC_TAG_SECRET}
        })
    end)
...
local function makeTag(plr, char)
    ...
    -- Remove existing tag from PlayerGui
    local oldBB = lpgui:FindFirstChild(plr.Name.."_AC_Tag")
    if oldBB then oldBB:Destroy() end

    local info = SPECIAL[plr.Name]
    local isSpecial = info ~= nil
    local tagText = (info and info.tag) or "AC USER"
    local bgColor = (info and info.bgColor) or C.purple

    -- BillboardGui parented to PlayerGui so all clients see via API
    local bb = Instance.new("BillboardGui")
    bb.Name = plr.Name.."_AC_Tag"
    ...
    bgFrame.BackgroundColor3 = isSpecial and bgColor or Color3.fromRGB(12, 12, 16)
    bgFrame.BackgroundTransparency = 0.2
    bgFrame.ZIndex = 1
    bgFrame.Parent = bb
    local bgC = Instance.new("UICorner"); bgC.CornerRadius = UDim.new(0.4, 0); bgC.Parent = bgFrame

    -- Animated gradient stroke
    local bgS = Instance.new("UIStroke")
    bgS.ApplyStrokeMode = "Border"
    bgS.Color = Color3.fromRGB(255, 255, 255)
    bgS.Thickness = 3
    bgS.Parent = bgFrame
    ...
    strokeGrad.Parent = bgS

    -- Background gradient fill
    local bgGrad = Instance.new("UIGradient")
    bgGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0,   isSpecial and bgColor or Color3.fromRGB(40, 0, 36)),
        ColorSequenceKeypoint.new(0.5, isSpecial and (info and info.bgColor or C.purpleDim) or Color3.fromRGB(12, 12, 16)),
        ColorSequenceKeypoint.new(1,   isSpecial and bgColor or Color3.fromRGB(40, 0, 36)),
    })
    bgGrad.Parent = bgFrame

    -- Background image (SPECIAL only)
    if isSpecial and info.bgId and info.bgId ~= "" then
    ...
    pfpImg.Name = "PFP"
    pfpImg.Size = UDim2.fromScale(0.215, 0.7)
    pfpImg.Position = UDim2.fromScale(0.05, 0.167)
    ...
    if isSpecial and info.pfpId and info.pfpId ~= "" then
    ...
    mainLbl.Name = "TagLabel"
    mainLbl.Size = UDim2.fromScale(0.7, 0.5)
    mainLbl.Position = UDim2.fromScale(0.29955, 0)
    mainLbl.BackgroundTransparency = 1
    mainLbl.Text = tagText
    ...
    mainLbl.TextYAlignment = Enum.TextYAlignment.Bottom
    mainLbl.TextWrapped = true
    mainLbl.ZIndex = 4
    ...
    -- Heartbeat: animate gradients + distance-based size scaling (like Scoper)
    local heartConn
    heartConn = RunService.Heartbeat:Connect(function(dt)
        if not bb.Parent then heartConn:Disconnect(); return end
        pcall(function()
            bb.Enabled = nameTagsEnabled

            -- Animate gradients
            strokeGrad.Rotation = (strokeGrad.Rotation + 120 * dt) % 360
            bgGrad.Rotation     = (bgGrad.Rotation     + 60  * dt) % 360

            -- Keep adornee updated on respawn
            if char and char:FindFirstChild("Head") then
                bb.Adornee = char.Head
            end

            -- Distance scaling (only for other players, not self)
            if plr ~= lp then
                ...
                if myChar and theirChar then
                    ...
                    if myHRP and theirHRP then
                        ...
                        local isFar = dist > 50
                        local currentlyFar = mainLbl.Visible == false

                        if isFar ~= currentlyFar then
                            mainLbl.Visible = not isFar
                            userLbl.Visible = not isFar

                            TweenService:Create(bb, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                                { Size = isFar and UDim2.new(1, 25, 1, 25) or UDim2.fromOffset(250, 75) }
                            ):Play()

                            TweenService:Create(pfpImg, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                                Position = isFar and UDim2.fromScale(0.25, 0.167) or UDim2.fromScale(0.05, 0.167),
                                Size     = isFar and UDim2.fromScale(0.5,  0.7)   or UDim2.fromScale(0.215, 0.7),
                            }):Play()
                        end
                    end
                end
...
-- API-based poll: register self, fetch who's running AC, tag them
local acSelfTagConn = nil

local function AC_ApplyTagsFromCache()
    for _, plrObj in ipairs(Players:GetPlayers()) do
        task.spawn(function()
            local entry = AC_DatabaseCache[tostring(plrObj.UserId)]
            if not entry or entry.executed ~= true then
                local existing = lpgui:FindFirstChild(plrObj.Name.."_AC_Tag")
                if existing then existing:Destroy() end
                AC_TaggedPlayers[plrObj.UserId] = nil
                activeTags[plrObj.UserId] = nil
                return
            end
            local tag = entry.tag or "AC USER"
            if AC_TaggedPlayers[plrObj.UserId] == tag then return end
            local char = plrObj.Character
            if not char then plrObj.CharacterAdded:Wait() end
            char = plrObj.Character
            if char and nameTagsEnabled then
                makeTag(plrObj, char)
                AC_TaggedPlayers[plrObj.UserId] = tag
            end
        end)
    end
end

-- Register self on execute + tag own character
task.spawn(function()
    AC_RegisterSelf()

    local function tagSelf(char)
        if acUnloaded or not char then return end
        char:WaitForChild("HumanoidRootPart", 5)
        if acUnloaded then return end
        task.wait(0.3)
        if nameTagsEnabled then pcall(function() makeTag(lp, char) end) end
    end
    tagSelf(lp.Character)
    acSelfTagConn = lp.CharacterAdded:Connect(tagSelf)
end)

-- Poll Vercel every 15s to update who is running AC
task.spawn(function()
    task.wait(1.5)
    while not acUnloaded do
        pcall(function()
            local db = AC_FetchAllTags()
            if db then
                AC_DatabaseCache = db
                AC_ApplyTagsFromCache()
            end
        end)
        task.wait(15)
    end
...
-- New player joined: check cache, tag if they're in it
Players.PlayerAdded:Connect(function(target)
    task.wait(1)
    local entry = AC_DatabaseCache[tostring(target.UserId)]
    if entry and entry.executed == true then
        local char = target.Character
        if not char then target.CharacterAdded:Wait() end
        char = target.Character
        if char then
            makeTag(target, char)
            AC_TaggedPlayers[target.UserId] = entry.tag or "AC USER"
        end
    end
    target.CharacterAdded:Connect(function(char)
        task.wait(0.5)
        if AC_TaggedPlayers[target.UserId] then
            makeTag(target, char)
        end
    end)
end)

-- Player left: clear from DB cache
Players.PlayerRemoving:Connect(function(target)
    if target == lp then AC_SetExecuted(false) end
    AC_TaggedPlayers[target.UserId] = nil
    AC_DatabaseCache[tostring(target.UserId)] = nil
    activeTags[target.UserId] = nil
end)

-- Spectator broadcast values (used by spectator HUD system)
...
-- Pre-declare flags so they're accessible outside the do block below
end -- TOOLS TAB scope
...
-- forward declarations for open functions
local openAnimLogger,openAutoClicker,openAntiBang,openAntiFling
local openAntiAFK,openBpColor,openChatColor,openFaceBang
local openHug,openFlingUI,openShiftLock,openTouchFling,openBaseplate

do -- wrap open* functions in own scope to stay under Luau 200-register limit
...
end -- end do block for open* functions

-- -- MISC TAB CONTENT -----------------------------------------
-- Hoisted to avoid exceeding 200 local register limit inside do block
local _miscInvisActive=false; local _miscInvisParts={}; local _miscInvisConn=nil
local _flyActive=false; local _flyBG=nil; local _flyBV=nil; local _flyKD=nil; local _flyKU=nil; local _flyHB=nil

local function toggleSupermanFly()
    if _flyActive then
        _flyActive=false
        pcall(function()
...
do
    ...
    local invisActive=_miscInvisActive; local invisParts=_miscInvisParts; local invisConn=_miscInvisConn
    local function cacheInvis()
        _miscInvisParts={}; invisParts=_miscInvisParts; local char=lp.Character; if not char then return end
        for _,p in ipairs(char:GetDescendants()) do if p:IsA("BasePart") and p.Transparency==0 then table.insert(invisParts,p) end end
    end
    cacheInvis(); lp.CharacterAdded:Connect(function() task.wait(1); _miscInvisActive=false; invisActive=false; cacheInvis() end)
    ...
    mkMiscCard("Invisible",     function(state)
        _miscInvisActive=state; invisActive=state
        local char=lp.Character; if not char then return end
        local hum=char:FindFirstChildOfClass("Humanoid"); local hrp=char:FindFirstChild("HumanoidRootPart")
        if not hum or not hrp then return end
        for _,p in ipairs(invisParts) do pcall(function() p.Transparency=invisActive and 0.5 or 0 end) end
        if invisActive then
            if _miscInvisConn then _miscInvisConn:Disconnect() end
            _miscInvisConn=RunService.Heartbeat:Connect(function()
                if not _miscInvisActive then _miscInvisConn:Disconnect(); return end
                pcall(function()
        ...
        else if _miscInvisConn then _miscInvisConn:Disconnect(); _miscInvisConn=nil end end
    end, true)
    ...
    mkMiscCard("Superman Fly", function() toggleSupermanFly() end)
