...
if type(getgenv().AudioCrafterLoaded) == "boolean" and getgenv().AudioCrafterLoaded then
    return
end

getgenv().AudioCrafterLoaded = true
...
repeat task.wait()
    ...
    -- -- NAMETAG SYSTEM (Scoper-style) --------------------------------
    -- URLs + state
    local NAMETAG_URL     = "https:".._sl.."yourscoper.vercel.app/api/nametag"
    local USERTAGS_URL    = "https:".._sl.."yourscoper.vercel.app/scripts/bin/usertags.json"
    local USERTAGSDATA_URL= "https:".._sl.."yourscoper.vercel.app/scripts/bin/usertagsdata.lua"
    local NT_SECRET       = ""
    ...
    local DEFAULT_STYLE = {
        primary  = Color3.fromRGB(72, 0, 66),
        useimage = false,
        accent   = ColorSequence.new({
            ColorSequenceKeypoint.new(0,   Color3.fromRGB(120, 0, 110)),
            ColorSequenceKeypoint.new(0.5, Color3.fromRGB(40,  0,  36)),
            ColorSequenceKeypoint.new(1,   Color3.fromRGB(200, 0, 180)),
        }),
        image = "http:".._sl.."www.roblox.com/asset/?id=84826198167974"
    }

    -- Fetch tag style data (rank colours/images) from server
    local function FetchTagStyleData()
        pcall(function()
            local ok,result=pcall(function()
                return Request({Url=USERTAGSDATA_URL,Method="GET"})
            end)
            if ok and result and result.Body then
                local ok2,data=pcall(function() return loadstring(result.Body)() end)
                if ok2 and data and data.RankData then TagStyleData=data.RankData end
            end
        end)
    end

    -- Fetch which userId maps to which tag name
    local function FetchUserTagsData()
        pcall(function()
            local ok,result=pcall(function()
                return Request({Url=USERTAGS_URL,Method="GET"})
            end)
            if ok and result and result.Body then
                local ok2,data=pcall(function() return HttpService:JSONDecode(result.Body) end)
                if ok2 and data then UserTagsData=data end
            end
        end)
    end

    local function GetTagFromWhitelist(userId)
        for tagName,userIds in pairs(UserTagsData) do
            for _,id in ipairs(userIds) do
                if id==userId then return tagName end
            end
        end
        return nil
    end
    ...
    local function FetchAllTags()
        local ok,result=pcall(function()
            return Request({
                Url=NAMETAG_URL.."?secret="..NT_SECRET,Method="GET",
                Headers={["x-secret"]=NT_SECRET}
            })
        end)
        if ok and result and result.Body then
            local ok2,data=pcall(function() return HttpService:JSONDecode(result.Body) end)
            if ok2 and data and data.nametags then return data.nametags end
        end
        return nil
    end

    local function SetExecuted(isExecuted)
        pcall(function()
            Request({
                Url=NAMETAG_URL.."?secret="..NT_SECRET, Method="POST",
                Headers={["x-secret"]=NT_SECRET,["Content-Type"]="application/json"},
                Body=HttpService:JSONEncode({userId=tostring(lp.UserId),executed=isExecuted})
            })
        end)
    end

    local function RegisterSelf()
        local tag = GetTagFromWhitelist(lp.UserId) or "AC USER"
        pcall(function()
            Request({
                Url=NAMETAG_URL.."?secret="..NT_SECRET, Method="POST",
                Headers={["x-secret"]=NT_SECRET,["Content-Type"]="application/json"},
                Body=HttpService:JSONEncode({
                    userId=tostring(lp.UserId), displayName=lp.Name,
                    tag=tag, executed=true
                })
            })
        end)
    end

    local function GetStyle(tag)
        return TagStyleData[tag] or DEFAULT_STYLE
    end

    -- Mute list: userId -> true means muted (VC audio suppressed)
    local MutedPlayers = {}

    -- SPECIAL table: custom tag text + asset IDs per username
    local SPECIAL_ASSETS = {
        ...
        ["xnearfeds"]           = {tag="AC USER",          pfpId="",               bgId=""},
    }
    ...
    local function makeTag(plr, char)
        ...
        -- Remove old tag
        local oldGui = lpgui:FindFirstChild(plr.Name.."_ACTag")
        if oldGui then oldGui:Destroy() end

        -- Resolve tag text and asset info
        local specialInfo  = SPECIAL_ASSETS[plr.Name]
        local serverEntry  = DatabaseCache[tostring(plr.UserId)]
        local tagText      = (specialInfo and specialInfo.tag)
            or (serverEntry and serverEntry.tag)
            or "AC USER"
        local style = GetStyle(tagText)

        -- BillboardGui in PlayerGui (survives respawn)
        local bill = Instance.new("BillboardGui")
        bill.Name                  = plr.Name.."_ACTag"
        bill.Active                = true
        bill.AlwaysOnTop           = true
        bill.ClipsDescendants      = false
        bill.LightInfluence        = 1
        bill.Enabled               = nameTagsEnabled
        ...
        bill.Parent                = lpgui
        activeTags[plr.UserId]     = bill

        -- Main frame
        local frame = Instance.new("Frame")
        frame.Name                   = "NameTagMainFrame"
        frame.BackgroundColor3       = (specialInfo and specialInfo.bgColor) or style.primary
        frame.BackgroundTransparency = 0.25
        frame.Size                   = UDim2.fromScale(1,1)
        frame.Parent                 = bill
        local frameCorner = Instance.new("UICorner")
        frameCorner.CornerRadius = UDim.new(0.4,0); frameCorner.Parent = frame

        -- Background image (bgId asset) if the special entry has one
        if specialInfo and specialInfo.bgId and specialInfo.bgId ~= "" then
            local bgImg = Instance.new("ImageLabel")
            bgImg.Size               = UDim2.fromScale(1,1)
            bgImg.BackgroundTransparency = 1
            bgImg.Image              = "rbxthumb:".._sl.."type=Asset&id="..specialInfo.bgId.."&w=420&h=420"
            bgImg.ImageTransparency  = 0
            bgImg.ScaleType          = Enum.ScaleType.Stretch
            bgImg.ZIndex             = 2
            bgImg.Parent             = frame
            local bgImgC = Instance.new("UICorner"); bgImgC.CornerRadius=UDim.new(0.4,0); bgImgC.Parent=bgImg
        end

        -- Background gradient (from style or AC purple default)
        local frameBG = Instance.new("UIGradient")
        frameBG.Color  = style.accent
        ...
        pfpCorner.CornerRadius = UDim.new(1,0); pfpCorner.Parent = pfp
        ...
        pfpStroke.Thickness = 2; pfpStroke.Parent = pfp

        -- Load pfp: pfpId uses rbxthumb (custom asset); fallback = Roblox avatar headshot
        task.spawn(function()
            if specialInfo and specialInfo.pfpId and specialInfo.pfpId ~= "" then
                -- Use the custom asset image directly – rbxthumb handles it reliably
                pfp.Image = "rbxthumb:".._sl.."type=Asset&id="..specialInfo.pfpId.."&w=150&h=150"
            else
                -- No custom asset – load live Roblox avatar headshot
        ...
        -- Mute button (only on other players' tags)
        if plr ~= lp then
            ...
            muteBtn.ZIndex               = 30
            muteBtn.Parent               = bill   -- parented to bill so it's always on top of frame
            local muteBtnCorner = Instance.new("UICorner")
            muteBtnCorner.CornerRadius = UDim.new(1,0); muteBtnCorner.Parent = muteBtn
            ...
            muteBtnStroke.Thickness = 2; muteBtnStroke.Parent = muteBtn

            -- Toggle mute on click
            muteBtn.MouseButton1Click:Connect(function()
                if MutedPlayers[plr.UserId] then
                    MutedPlayers[plr.UserId] = nil
                    muteBtn.Image = "rbxasset://textures/ui/VoiceChat/MicLight/Unmuted0.png"
                else
                    MutedPlayers[plr.UserId] = true
                    muteBtn.Image = "rbxasset://textures/ui/VoiceChat/MicLight/Muted.png"
                end
            end)

            -- Heartbeat: gradients, adornee, distance LOD (mute btn + labels hide beyond 50 studs)
            RunService.Heartbeat:Connect(function(dt)
                ...
                pcall(function()
                    ...
                    -- Sync mute icon
                    muteBtn.Image = MutedPlayers[plr.UserId]
            ...
            -- Click frame to TP behind target (mute button click is handled separately above)
            local _muteBtnHovered = false
            muteBtn.MouseEnter:Connect(function() _muteBtnHovered = true end)
            muteBtn.MouseLeave:Connect(function() _muteBtnHovered = false end)
            frame.InputBegan:Connect(function(input)
                if _muteBtnHovered then return end  -- don't TP when clicking the mute button
    ...
    -- -- OWN MIC BAR (sits beside Roblox's native VC button) --------------------
    -- Roblox places its VC mic button at bottom-right. We put our button
    -- right next to it using a ScreenGui anchored to the same corner.
    task.spawn(function()
        task.wait(1)  -- wait for Roblox VC UI to load
        -- Find Roblox's native VC frame to anchor next to it
        local function getAudioInput()
            local adi = nil
            pcall(function() adi = lp:FindFirstChildOfClass("AudioDeviceInput") end)
            if not adi then pcall(function()
            ...
            if not adi then pcall(function()
                for _,v in ipairs(game:GetDescendants()) do
                    if v.ClassName=="AudioDeviceInput" and v:IsDescendantOf(lp) then adi=v; break end
                end
            end) end
            return adi
        end
        ...
        micGui.Name            = "AC_MicBar"
        micGui.ResetOnSpawn    = false
        micGui.DisplayOrder    = 999
        micGui.IgnoreGuiInset  = true
        micGui.ZIndexBehavior  = Enum.ZIndexBehavior.Sibling
        micGui.Enabled         = true
        micGui.Parent          = lpgui

        -- Sit directly to the LEFT of the spec HUD anchor
        -- specAnchor is at UDim2.new(1,-234,0,116), size 220x36
        -- We place our 36x36 circle at x = specAnchor.left - gap = (1,-234-36-6) = (1,-276)
        -- vertically centred on the same row: y = 116 + (36-36)/2 = 116
        local micBar = Instance.new("Frame")
        micBar.Name                  = "MicBar"
        micBar.Size                  = UDim2.fromOffset(36, 36)
        micBar.Position              = UDim2.new(1, -276, 0, 116)
        micBar.AnchorPoint           = Vector2.new(0, 0)
        micBar.BackgroundColor3      = Color3.fromRGB(12,12,16)
        micBar.BackgroundTransparency= 0.15
        micBar.BorderSizePixel       = 0
        micBar.Parent                = micGui
        local micBarCorner = Instance.new("UICorner")
        micBarCorner.CornerRadius = UDim.new(1,0); micBarCorner.Parent = micBar

        -- Rotating purple gradient border (same as UI panels)
        local micBarStroke = Instance.new("UIStroke")
        micBarStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
        micBarStroke.Thickness       = 3; micBarStroke.Parent = micBar
        local micBarGrad = Instance.new("UIGradient")
        micBarGrad.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0,    Color3.fromRGB(255,255,255)),
            ColorSequenceKeypoint.new(0.25, Color3.fromRGB(120,0,150)),
            ColorSequenceKeypoint.new(0.5,  Color3.fromRGB(10,10,10)),
            ColorSequenceKeypoint.new(0.75, Color3.fromRGB(120,0,150)),
            ColorSequenceKeypoint.new(1,    Color3.fromRGB(255,255,255)),
        })
        micBarGrad.Parent = micBarStroke
        RunService.Heartbeat:Connect(function(dt2)
            if micBarStroke.Parent then
                micBarGrad.Rotation = (micBarGrad.Rotation + 120*dt2) % 360
            end
        end)

        -- Mic icon button
        local micIcon = Instance.new("ImageButton")
        micIcon.Size                  = UDim2.fromScale(0.72, 0.72)
        micIcon.Position              = UDim2.fromScale(0.14, 0.14)
        micIcon.BackgroundTransparency= 1
        micIcon.Image                 = "rbxasset://textures/ui/VoiceChat/MicLight/Unmuted0.png"
        micIcon.ZIndex                = 5
        micIcon.AutoButtonColor       = false
        micIcon.Parent                = micBar

        -- Read initial mic state
        local selfMuted = false
        task.spawn(function()
            task.wait(0.5)
            local adi = getAudioInput()
            if adi then
                selfMuted = adi.Muted
                micIcon.Image = selfMuted
                    and "rbxasset://textures/ui/VoiceChat/MicLight/Muted.png"
                    or  "rbxasset://textures/ui/VoiceChat/MicLight/Unmuted0.png"
                adi:GetPropertyChangedSignal("Muted"):Connect(function()
                    selfMuted = adi.Muted
                    micIcon.Image = selfMuted
                        and "rbxasset://textures/ui/VoiceChat/MicLight/Muted.png"
                        or  "rbxasset://textures/ui/VoiceChat/MicLight/Unmuted0.png"
        ...
        micIcon.MouseButton1Click:Connect(function()
            selfMuted = not selfMuted
            micIcon.Image = selfMuted
                and "rbxasset://textures/ui/VoiceChat/MicLight/Muted.png"
                or  "rbxasset://textures/ui/VoiceChat/MicLight/Unmuted0.png"
            micBar.BackgroundColor3 = selfMuted
                and Color3.fromRGB(160,0,0)
                or  Color3.fromRGB(12,12,16)
            pcall(function()
                local adi = getAudioInput()
                if adi then adi.Muted = selfMuted end
            end)
        end)
        -- Expose so Anti-VC Ban card can show/hide it
        getgenv().AC_MicGui = micGui
    end)
    -- -- END OWN MIC BAR --------------------------------------------------------
    ...
    -- Register self with Scoper API + start poll loop
    ...
    task.spawn(function()
        if Request then
            ...
            if db then
                DatabaseCache = db
                -- Apply server tags to anyone already in the server
                for _,plrObj in ipairs(Players:GetPlayers()) do
                    task.spawn(function()
                        local entry = DatabaseCache[tostring(plrObj.UserId)]
                        if entry and entry.executed == true then
                            if not plrObj.Character then plrObj.CharacterAdded:Wait() end
                            local char = plrObj.Character
                            if char then
                                local tag = activeTags[plrObj.UserId]
                                local head = char:FindFirstChild("Head")
                                if nameTagsEnabled and head and (not tag or not tag.Parent) then
                                    makeTag(plrObj, char)
                                end
                            end
                        end
    ...
    -- Poll every 2s for AC script users (workspace StringValues) + refresh tags
    task.spawn(function()
        while not acUnloaded do
            ...
            pcall(function()
                ...
                for uid,tag in pairs(activeTags) do
                    if not tag or not tag.Parent then activeTags[uid]=nil
                    elseif not currentPlayers[uid] then pcall(function() tag:Destroy() end); activeTags[uid]=nil
                    end
                end
            end)
        end
    ...
    -- PlayerAdded: tag newcomers if they have a server entry or AC script
    Players.PlayerAdded:Connect(function(target)
        task.wait(1)
        local entry = DatabaseCache[tostring(target.UserId)]
        if entry and entry.executed==true and entry.tag then
            if not target.Character then target.CharacterAdded:Wait() end
            local char = target.Character
            if char then makeTag(target, char) end
        end
        target.CharacterAdded:Connect(function(char)
            task.wait(0.5)
            if activeTags[target.UserId] then makeTag(target, char) end
        end)
    end)

    -- PlayerRemoving: clean up
    Players.PlayerRemoving:Connect(function(target)
        if target == lp then SetExecuted(false) end
        activeTags[target.UserId] = nil
        DatabaseCache[tostring(target.UserId)] = nil
        pcall(function()
            local g = lpgui:FindFirstChild(target.Name.."_ACTag")
    ...
    -- -- MIC OVERLAY SYSTEM ----------------------------------------
    -- Floating mic icon above every player's head (higher than nametag)
    -- Click to mute/unmute their AudioDeviceInput. Works on ALL players.
    local micOverlays   = {}  -- [userId] = BillboardGui
    local micMuted      = {}  -- [userId] = bool (our local mute state per player)

    local MIC_MUTED    = "rbxasset://textures/ui/VoiceChat/MicLight/Muted.png"
    local MIC_UNMUTED  = "rbxasset://textures/ui/VoiceChat/MicLight/Unmuted0.png"

    -- Find an AudioDeviceInput belonging to a player
    ...
    local function makeMicOverlay(plr)
        if micOverlays[plr.UserId] and micOverlays[plr.UserId].Parent then return end
        ...
        -- BillboardGui: sits higher than the nametag (nametag is at 2.5, this at 4.5)
        local bill = Instance.new("BillboardGui")
        bill.Name                  = plr.Name.."_ACMic"
        bill.Active                = true
        bill.AlwaysOnTop           = true
        bill.ClipsDescendants      = false
        bill.LightInfluence        = 1
        bill.Size                  = UDim2.fromOffset(28, 28)
        bill.StudsOffsetWorldSpace = Vector3.new(0, 4.8, 0)
        bill.ResetOnSpawn          = false
        bill.Adornee               = head
        bill.ZIndexBehavior        = Enum.ZIndexBehavior.Sibling
        bill.Parent                = lpgui
        micOverlays[plr.UserId]    = bill

        -- Dark circle background (no background color, transparent so glow border shines)
        ...
        bgCorner.CornerRadius = UDim.new(1,0); bgCorner.Parent = bg

        -- Exact same rotating gradient glow as the main UI panel border
        local bgStroke = Instance.new("UIStroke")
        bgStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
        bgStroke.Thickness       = 3.5; bgStroke.Parent = bg
        local bgSGrad = Instance.new("UIGradient")
        bgSGrad.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0,    Color3.fromRGB(255,255,255)),
            ColorSequenceKeypoint.new(0.25, Color3.fromRGB(120,0,150)),
            ColorSequenceKeypoint.new(0.5,  Color3.fromRGB(10,10,10)),
            ColorSequenceKeypoint.new(0.75, Color3.fromRGB(120,0,150)),
        ...
        bgSGrad.Parent = bgStroke
        RunService.Heartbeat:Connect(function(dt2)
            if bgStroke.Parent then bgSGrad.Rotation=(bgSGrad.Rotation+120*dt2)%360 end
        end)

        local micBtn = Instance.new("ImageButton")
        micBtn.Size                  = UDim2.fromScale(0.72, 0.72)
        micBtn.Position              = UDim2.fromScale(0.14, 0.14)
        micBtn.BackgroundTransparency= 1
        micBtn.Image                 = micMuted[plr.UserId] and MIC_MUTED or MIC_UNMUTED
        micBtn.ZIndex                = 5
        micBtn.AutoButtonColor       = false
        micBtn.Parent                = bill

        -- Click: toggle mute
        micBtn.MouseButton1Click:Connect(function()
            micMuted[plr.UserId] = not micMuted[plr.UserId]
            local isMuted = micMuted[plr.UserId]
            micBtn.Image = isMuted and MIC_MUTED or MIC_UNMUTED
            pcall(function()
                local tagGui = lpgui:FindFirstChild(plr.Name.."_ACTag")
                if tagGui then
                    local muteInTag = tagGui:FindFirstChild("NameTagMuteButton", true)
                    if muteInTag then muteInTag.Image = isMuted and MIC_MUTED or MIC_UNMUTED end
                end
            end)
            pcall(function()
        ...
        -- Heartbeat: adornee + distance check (only show within 10 studs) + mute state
        RunService.Heartbeat:Connect(function(dt)
            if not bill.Parent then return end
            pcall(function()
                -- Track head
                ...
                -- Distance check: only show within 10 studs of local player
                -- Own mic is in the AC_MicBar ScreenGui, not here
                local visible = false
                if plr ~= lp
                ...
                -- Mute state: red bg when muted, dark when not
                local isMuted = micMuted[plr.UserId]
                micBtn.Image = isMuted and MIC_MUTED or MIC_UNMUTED
                bg.BackgroundColor3 = isMuted
                    and Color3.fromRGB(160,0,0)
                    or  Color3.fromRGB(12,12,16)
            end)
        end)
    ...
    local function removeMicOverlay(plr)
        pcall(function()
            local g = lpgui:FindFirstChild(plr.Name.."_ACMic")
            if g then g:Destroy() end
        end)
        micOverlays[plr.UserId] = nil
        micMuted[plr.UserId]    = nil
    end

    -- Spawn overlays for all current players (skip self — own mic is in AC_MicBar)
    task.spawn(function()
        task.wait(1)
    ...
    -- New players: create overlay; rebuild on respawn
    Players.PlayerAdded:Connect(function(plr)
        if plr == lp then return end
        plr.CharacterAdded:Connect(function()
            task.wait(0.5)
            removeMicOverlay(plr)
            makeMicOverlay(plr)
        end)
    ...
    -- Unload cleanup stored so unload button can call it
    getgenv().AC_RemoveAllMicOverlays = function()
        for _,plr in ipairs(Players:GetPlayers()) do
            removeMicOverlay(plr)
        end
    end
    -- -- END MIC OVERLAY SYSTEM -------------------------------------

    local specHudGui, specAnchor, specDot, specCount, specCards, pulseTween1, pulseTween2, watcherCards, statsGui, statsFrame, fpsVal, fpsIco, usrVal, usrIco, msVal, msIco, updateHudAccent
    local function _buildHUDs()
    -- -- SPECTATOR HUD ---------------------------------------------
    -- Always visible in top-right, slightly below corner
    specHudGui = Instance.new("ScreenGui")
    ...
    specDot=Instance.new("Frame"); specDot.Name="SpecDot"
    ...
    specCount=Instance.new("TextLabel"); specCount.Size=UDim2.new(1,-20,1,0)
    ...
    specCards=Instance.new("Frame"); specCards.Name="SpecCards"
    ...
    -- Pulse tweens
    pulseTween1=TweenService:Create(specDot,TweenInfo.new(0.8,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),{Size=UDim2.new(0,18,0,18),BackgroundTransparency=0.2})
    pulseTween2=TweenService:Create(specDot,TweenInfo.new(0.5,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),{BackgroundTransparency=0.6})
    pulseTween2:Play()
    ...
    statsGui=Instance.new("ScreenGui"); statsGui.Name="AC_StatsHud"
    ...
    statsFrame=Instance.new("Frame"); statsFrame.Name="StatsFrame"
    ...
    fpsVal, fpsIco   = mkStatCell("FPS", 4, 80)
    usrVal, usrIco   = mkStatCell("USR", 88, 80)
    msVal,  msIco    = mkStatCell("MS",  172, 80)
    ...
    local specPollConn = RunService.Heartbeat:Connect(function()
        ...
        if count > 0 then
            ...
            specCount.Text = tostring(count).." watching"
        else
            pulseTween1:Cancel(); pulseTween2:Play()
            specDot.BackgroundTransparency = 1
            specCount.Text = "0 watching"
        end
    end)
    end -- _buildHUDs
    _buildHUDs()

    -- Pre-declare Win-related vars accessible to tab builders
    local Header, UnloadBtn, NTagBtn, KeyBtn, CloseBtn, ContentArea, TabBar
    local pages, pageData, tabBtns
    local switchPage
    local openKey, listeningKey
    local WIN_FULL_SIZE, WIN_FULL_POS
    local openWin, closeWin  -- assigned inside _buildWinFrame

    local Win  -- pre-declared for global access
    local function _buildWinFrame()
    ...
    Header=mkFrame(Win,{Name="Header",Size=UDim2.new(1,0,0,HEADER_H),BackgroundColor3=Color3.fromRGB(0,0,0),BackgroundTransparency=1,ZIndex=5})
    ...
    UnloadBtn=mkBtn(Header,"Unload",{Size=UDim2.new(0,54,0,22),Position=UDim2.new(1,-257,0.5,-11),TextSize=10,Font=Enum.Font.GothamBold,ZIndex=6})
    ...
    UnloadBtn.Activated:Connect(function()
        ...
        -- 3. Remove all _ACTag BillboardGuis from PlayerGui
        pcall(function()
            for _,plr in ipairs(Players:GetPlayers()) do
                local g = lpgui:FindFirstChild(plr.Name.."_ACTag")
                if g then g:Destroy() end
            end
            -- clear activeTags table
            for uid,tag in pairs(activeTags) do
                pcall(function() if tag and tag.Parent then tag:Destroy() end end)
                activeTags[uid]=nil
            end
        end)
        -- 3c. Remove mic overlays + own mic bar
        pcall(function()
            if getgenv().AC_RemoveAllMicOverlays then
                getgenv().AC_RemoveAllMicOverlays()
            end
            local mb = lpgui:FindFirstChild("AC_MicBar")
    ...
    NTagBtn=mkBtn(Header,"Tags: ON",{Size=UDim2.new(0,58,0,22),Position=UDim2.new(1,-195,0.5,-11),TextSize=10,Font=Enum.Font.GothamBold,ZIndex=6})
    ...
    openKey=Enum.KeyCode.G; listeningKey=false
    local function keyName(kc) return (tostring(kc):match("KeyCode%.(.+)") or tostring(kc)) end

    KeyBtn=mkBtn(Header,"Key: G",{Size=UDim2.new(0,62,0,22),Position=UDim2.new(1,-130,0.5,-11),TextSize=10,Font=Enum.Font.Gotham,ZIndex=6})
    ...
    CloseBtn=mkBtn(Header,"x",{Size=UDim2.new(0,24,0,22),Position=UDim2.new(1,-56,0.5,-11),TextSize=13,Font=Enum.Font.GothamBold,ZIndex=6})
    ...
    WIN_FULL_SIZE = UDim2.new(0,WIN_W,0,WIN_H)
    WIN_FULL_POS  = UDim2.new(0,16,0.5,-WIN_H/2)

    function openWin()
        -- Start off-screen to the left, full size, then slide right into place
        Win.Size     = WIN_FULL_SIZE
        Win.Position = UDim2.new(0, -WIN_W - 20, 0.5, -WIN_H/2)
        Win.Visible  = true
        TweenService:Create(Win, TweenInfo.new(0.28, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
            {Position=WIN_FULL_POS}):Play()
    end
    function closeWin()
        -- Slide back off-screen to the left then hide
        local tw = TweenService:Create(Win, TweenInfo.new(0.22, Enum.EasingStyle.Quint, Enum.EasingDirection.In),
            {Position=UDim2.new(0, -WIN_W - 20, 0.5, -WIN_H/2)})
        tw:Play()
        tw.Completed:Connect(function()
            Win.Visible=false
            Win.Position=WIN_FULL_POS
        end)
    end
    CloseBtn.Activated:Connect(closeWin)
    ...
    UserInputService.InputBegan:Connect(function(inp,gp)
        ...
        elseif not listeningKey and inp.UserInputType==Enum.UserInputType.Keyboard and inp.KeyCode==openKey then
            if keyValidated then if Win.Visible then closeWin() else openWin() end end
        end
    end)

    ContentArea=mkFrame(Win,{Name="Content",Size=UDim2.new(1,0,0,CONTENT_H),Position=UDim2.new(0,0,0,HEADER_H),BackgroundColor3=Color3.fromRGB(0,0,0),BackgroundTransparency=1,ZIndex=2})
    mkFrame(Win,{Size=UDim2.new(1,0,0,1),Position=UDim2.new(0,0,1,-(TABBAR_H+1)),BackgroundColor3=C.sep,ZIndex=6})
    TabBar=mkFrame(Win,{Name="TabBar",BackgroundTransparency=1,Size=UDim2.new(1,0,0,TABBAR_H),Position=UDim2.new(0,0,1,-TABBAR_H),BackgroundColor3=Color3.fromRGB(0,0,0),ZIndex=5})
    do
    ...
    _buildWinFrame()
    -- Pre-declare panel openers so tab cards can call them before _buildEmotesSystems runs
    local openUGCPanel, openRNPanel, openAnimLogger, openAutoClicker
    local openAntiBang, openAntiFling, openAntiAFK, openChatColor
    local openFaceBang, openHug, openFlingUI, openShiftLock, openTouchFling
    local openAnimCopier
    -- openBaseplate and openBpColor already pre-declared above
    ...
    local function _buildEmotesSystems()
    ...
    local function openBpColor()
        ...
        -- Find ALL baseplates — aggressive scan: name match OR large flat anchored part
        local function getAllBaseplates()
            ...
            for _,v in pairs(workspace:GetDescendants()) do
                if v:IsA("BasePart") and not seen[v] then
                    ...
                    -- Name-based: any common floor/ground name
                    local nameMatch = name=="baseplate" or name=="base" or name=="floor"
                        or name=="ground" or name=="terrain" or name=="platform"
                        or name:find("base") or name:find("floor") or name:find("ground")
                    -- Size-based: flat anchored part wider than 20 studs on both axes
                    local isBig = v.Size.X>20 and v.Size.Z>20 and v.Size.Y<=20 and v.Anchored
                    if nameMatch or isBig then
                        seen[v]=true
                        table.insert(list,v)
                    end
                end
        ...
        -- Reset + Save buttons side by side
        local resetBtn=mkBtn(ct,"↺ Reset",{Size=UDim2.new(0.48,0,0,30),Position=UDim2.new(0,0,0,264),TextSize=11,Font=Enum.Font.GothamBold,ZIndex=72,BackgroundColor3=C.cardDark})
        corner(resetBtn,6); mkStroke(resetBtn,C.cardBord,1)
        hov(resetBtn,C.purpleDim,C.cardDark)
        resetBtn.Activated:Connect(function()
            local defaultCol=Color3.fromRGB(106,127,63)
            for _,bp in ipairs(getAllBaseplates()) do
                pcall(function() bp.Color=defaultCol; bp.BrickColor=BrickColor.new(defaultCol); bp.Material=Enum.Material.Grass end)
            end
            bpCurrentColor=defaultCol
            hu,sa,va=Color3.toHSV(defaultCol)
            hKnob.Position=UDim2.new(hu,-7,0.5,-7)
            sKnob.Position=UDim2.new(sa,-7,0.5,-7)
            vKnob.Position=UDim2.new(va,-7,0.5,-7)
            updateAll()
        end)

        local saveBtn=mkBtn(ct,"✓ Save",{Size=UDim2.new(0.48,0,0,30),Position=UDim2.new(0.52,0,0,264),TextSize=11,Font=Enum.Font.GothamBold,ZIndex=72,BackgroundColor3=C.purple})
        corner(saveBtn,6); mkStroke(saveBtn,C.cardBord,1)
        hov(saveBtn,C.purpleLight or C.purple,C.purpleDim)
        saveBtn.Activated:Connect(function()
            local col=Color3.fromHSV(hu,sa,va)
            bpCurrentColor=col
            for _,bp in ipairs(getAllBaseplates()) do
                pcall(function() bp.Color=col; bp.BrickColor=BrickColor.new(col); bp.Material=Enum.Material.SmoothPlastic end)
            end
            -- Flash save button green briefly
            saveBtn.BackgroundColor3=C.green
            task.delay(0.8, function() if saveBtn.Parent then saveBtn.BackgroundColor3=C.purple end end)
        end)
    ...
    -- shared color for baseplate coloring, used by openBpColor
    local bpCurrentColor = nil

    -- Returns ALL Baseplate-like parts in the workspace (any depth)
    local function findAllBaseplates()
        local found = {}
        local function scan(container)
            for _,v in ipairs(container:GetChildren()) do
                if v:IsA("BasePart") then
                    local nm = v.Name:lower()
                    if nm=="baseplate" or nm=="base" or nm=="floor" or nm=="ground" then
    ...
    -- Apply a Color3 to every known baseplate + our own ACBaseplate
    local function applyBpColor(col)
        bpCurrentColor = col
        pcall(function()
            for _,bp in ipairs(findAllBaseplates()) do
                bp.BrickColor = BrickColor.new(col)
                bp.Material   = Enum.Material.SmoothPlastic
            end
        end)
        if basePart and basePart.Parent then
            basePart.BrickColor = BrickColor.new(col)
        end
    end

    openBaseplate=function()
        if baseplateOpen then return end; baseplateOpen=true
        local panel,ct,cls=mkStdPanel("Baseplate",260,130,"Infinite Baseplate")
        cls.Activated:Connect(function()
            if basePart then basePart:Destroy(); basePart=nil end
            panel:Destroy(); baseplateOpen=false
        end)
        local active=false
        local statusLbl6=mkLabel(ct,"Baseplate: Inactive",{Size=UDim2.new(1,0,0,16),TextSize=11,Font=Enum.Font.GothamBold,TextColor3=C.txtDim,ZIndex=72})
        local aBtn3=mkBtn(ct,"Click to Activate",{
            Size=UDim2.new(1,0,0,34),Position=UDim2.new(0,0,0,22),
            BackgroundColor3=C.cardDark,TextSize=12,Font=Enum.Font.GothamBold,ZIndex=72})
        corner(aBtn3,8); mkStroke(aBtn3,C.cardBord,1)
        aBtn3.MouseEnter:Connect(function()
            TweenService:Create(aBtn3,TweenInfo.new(0.05),{BackgroundColor3=C.card}):Play() end)
        aBtn3.MouseLeave:Connect(function()
            TweenService:Create(aBtn3,TweenInfo.new(0.05),{BackgroundColor3=active and C.green or C.cardDark}):Play() end)
        aBtn3.Activated:Connect(function()
            active = not active
            if active then
                aBtn3.Text="Activated"; aBtn3.BackgroundColor3=C.green
                statusLbl6.Text="Baseplate: Active"; statusLbl6.TextColor3=C.green
                -- Recolor all existing baseplates in the game
                if bpCurrentColor then applyBpColor(bpCurrentColor) end
                -- Spawn our invisible floor that follows the player
                if basePart then basePart:Destroy() end
                basePart=Instance.new("Part"); basePart.Name="ACBaseplate"
                basePart.Size=Vector3.new(20000,10,20000); basePart.Transparency=1
                basePart.Anchored=true; basePart.CanCollide=true
                basePart.CastShadow=false; basePart.Material=Enum.Material.SmoothPlastic
                basePart.Parent=workspace
                local chr0=lp.Character
                local rp0=chr0 and chr0:FindFirstChild("HumanoidRootPart")
                local fixedY=(rp0 and (rp0.Position.Y-8)) or -5
                basePart.CFrame=CFrame.new(rp0 and rp0.Position.X or 0,fixedY,rp0 and rp0.Position.Z or 0)
                RunService.RenderStepped:Connect(function()
                    if not basePart or not basePart.Parent or not active then return end
                    local chr=lp.Character; if not chr then return end
                    local rp7=chr:FindFirstChild("HumanoidRootPart"); if not rp7 then return end
                    basePart.CFrame=CFrame.new(rp7.Position.X,fixedY,rp7.Position.Z)
                end)
            else
                aBtn3.Text="Click to Activate"; aBtn3.BackgroundColor3=C.cardDark
                statusLbl6.Text="Baseplate: Inactive"; statusLbl6.TextColor3=C.txtDim
                if basePart then basePart:Destroy(); basePart=nil end
            end
        end)
    end
    ...
    -- Misc tab: lift heavy-local features to module scope to stay under 200-register limit
    -- All fly state in one table = 1 register instead of 6
    local _FV={active=false,bg=nil,bv=nil,kd=nil,ku=nil,hb=nil}

    local function _miscFlyToggle()
        if _FV.active then
            _FV.active=false
            pcall(function()
                ...
                if _FV.bg then _FV.bg:Destroy(); _FV.bg=nil end
                if _FV.bv then _FV.bv:Destroy(); _FV.bv=nil end
                if _FV.kd then _FV.kd:Disconnect(); _FV.kd=nil end
                if _FV.ku then _FV.ku:Disconnect(); _FV.ku=nil end
                if _FV.hb then _FV.hb:Disconnect(); _FV.hb=nil end
            end)
            return
        end
        local char=lp.Character; if not char then return end
        ...
        local function playFlyAnim(id,t,s)
            if _lastAnim==id then return end
            _lastAnim=id
            pcall(function()
                char.Animate.Disabled=false
                ...
                _lastTrack.Stopped:Connect(function() char.Animate.Disabled=false end)
            end)
        end
        local bg=Instance.new("BodyGyro",hrp); bg.P=9e4; bg.MaxTorque=Vector3.new(9e9,9e9,9e9); bg.CFrame=hrp.CFrame; _FV.bg=bg
        local bv=Instance.new("BodyVelocity",hrp); bv.Velocity=Vector3.new(0,0.1,0); bv.MaxForce=Vector3.new(9e9,9e9,9e9); _FV.bv=bv
        playFlyAnim("10714347256",4,0)
        _FV.kd=lp:GetMouse().KeyDown:connect(function(key)
        ...
        _FV.ku=lp:GetMouse().KeyUp:connect(function(key)
        ...
        _FV.hb=RunService.Heartbeat:Connect(function()
            if not _FV.active then _FV.hb:Disconnect(); _FV.hb=nil; return end
            hum.PlatformStand=true
    ...
    -- Invisible toggle using LocalTransparencyModifier (works on modern Roblox executors)
    local _IV = {active=false, conn=nil}

    local function _applyInvis(char, invisible)
        if not char then return end
        ...
        for _, a in ipairs(char:GetChildren()) do
            if a:IsA("Accessory") then
                ...
                if handle then
                    pcall(function() handle.LocalTransparencyModifier = invisible and 1 or 0 end)
                end
            end
        end
    end

    local function _miscInvisToggle(state)
        _IV.active = state
        local char = lp.Character
        _applyInvis(char, state)

        if state then
            -- Keep invisible through re-renders and respawns
            if _IV.conn then _IV.conn:Disconnect() end
            _IV.conn = RunService.RenderStepped:Connect(function()
                if not _IV.active then _IV.conn:Disconnect(); _IV.conn=nil; return end
                local c = lp.Character
                if c then _applyInvis(c, true) end
            end)
        else
            if _IV.conn then _IV.conn:Disconnect(); _IV.conn=nil end
            -- Restore all parts
            local c = lp.Character
            if c then _applyInvis(c, false) end
        end
    end
    ...
    lp.CharacterAdded:Connect(function(char)
        task.wait(0.5)
        _IV.active = false  -- reset state on respawn
    end)
    ...
    local function _buildMiscTab()
        ...
        mkMiscCard("Invisible", _miscInvisToggle, true)
        ...
        mkMiscCard("Superman Fly", _miscFlyToggle)
        mkMiscCard("Touch Fling",   function() openTouchFling() end)
        mkMiscCard("FPS Boost", function()
            pcall(function()
                -- Reduce shadow/reflection quality for extra FPS
                ...
                -- Destroy idle non-playing sounds to free audio resources
                for _,v in pairs(workspace:GetDescendants()) do
                    pcall(function()
                        if v:IsA("Sound") and not v.IsPlaying then v:Destroy() end
                    end)
                end
                -- (QualityLevel is read-only in-game, skipped)
            end)
        end)
    ...
    _buildAdminTab()
    -- -- COMMAND SYSTEM --------------------------------------------
    local function _buildCommandSystem()
    ...
    local COMMANDS = {
        {cmd=".rj", desc="Rejoin same server", run=function()
            task.spawn(function()
                ...
                pcall(function()
                    if jobId then
                        TS2:TeleportToPlaceInstance(game.PlaceId, jobId, lp)
                    end
                end)
            end)
        end},
    ...
    local function _buildKeySystem()
        ...
        local function grantAccess(cachedKey)
            ...
            task.wait(0.4); keyGui:Destroy(); openWin(); switchPage(1)
        end
        ...
        if OWNER_USERS[lp.Name] then
            ...
            keyGui:Destroy(); keyValidated=true; registerSelf(); openWin(); switchPage(1)
            task.spawn(function()
    ...
    Players.PlayerRemoving:Connect(function(Parameter)
        if not acUnloaded then
            if Parameter == lp then
                for _ = 1, 2 do
                    QueueTeleport('loadstring(game:HttpGet("https://audio-crafter.vercel.app/api/audiocrafter"))()')
                end
            end
        end
    end)
